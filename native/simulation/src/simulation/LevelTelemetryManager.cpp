...
auto aggregate_timings(std::vector<double> samples) -> FLevelTelemetryTimingAggregate {
...
void FLevelTelemetryManager::record_simulation_tick_timing(
    ...
    for (std::int32_t index{}; index < FSimulationTelemetryPerformanceWindow::system_count;
         ++index) {
    ...
    for (std::int32_t index{}; index < FSimulationTelemetryPerformanceWindow::phase_count;
         ++index) {
        if (phases[index] >= 0.0) {
...
void FLevelTelemetryManager::sample_battle_state(bool const force) {
    ...
    samples.push_back(FLevelTelemetryBattleSample{
        .completed_tick = tick,
...
void FLevelTelemetryManager::mark_mission_terminal(FLevelMissionResult const& result) {
    auto const reason{result.state == ml::simulation::MissionState::Succeeded
                          ? ml::simulation::LevelTelemetryRunEndReason::MissionSucceeded
                          : ml::simulation::LevelTelemetryRunEndReason::MissionFailed};
    finalize_run(reason, false, {}, &result, ml::monotonic_seconds());
}

void FLevelTelemetryManager::finalize_interrupted(
    ml::simulation::LevelTelemetryRunEndReason const reason, std::string world_end_reason) {
    finalize_run(reason, true, std::move(world_end_reason), nullptr, ml::monotonic_seconds());
}

void FLevelTelemetryManager::finalize_completed(
...
auto FLevelTelemetryManager::take_finalized_run() -> std::optional<FLevelTelemetryRunRecord> {
...
void FLevelTelemetryManager::finalize_run(ml::simulation::LevelTelemetryRunEndReason const reason,
