...
[[nodiscard]] auto validate_channel(TranslationChannelConfig const& config) noexcept
    -> std::expected<void, FlightModelConfigError> {
...
[[nodiscard]] auto validate_drive(TranslationDriveConfig const& config) noexcept
    -> std::expected<void, FlightModelConfigError> {
    for (auto const value : {config.positive_speed_limit,
...
[[nodiscard]] auto validate_axis(TranslationAxisConfig const& config) noexcept
    -> std::expected<void, FlightModelConfigError> {
    if (auto const result{validate_channel(config.manual)}; !result) {
        return result;
    }
    if (auto const result{validate_channel(config.automatic)}; !result) {
        return result;
    }
...
[[nodiscard]] auto make_starfox_config() -> FlightModelConfig {
    ...
    forward.automatic.automatic_value = 12000.f;
