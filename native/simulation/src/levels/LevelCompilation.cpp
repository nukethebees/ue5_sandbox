...
namespace {
struct EventSource {
    simulation::SimTick execution_tick{};
...
auto append_mission_groups(FLevelEventSchedule& schedule,
                           MissionTickValues& values,
                           std::string& error) -> bool {
    auto const append = [&](simulation::LevelMissionEventType const type,
        ...
        error = "Level event compilation at tick " +
                ...
                std::to_string(std::numeric_limits<simulation::LevelEventCount>::max());
        return false;
    };
    if (!append(
            simulation::LevelMissionEventType::MustSurvive, values.must_survive, "must-survive") ||
        !append(simulation::LevelMissionEventType::RequiredKill,
                values.required_kills,
                "required-kill") ||
        !append(simulation::LevelMissionEventType::IncreaseKillTarget,
...
auto append_spawn_groups(FLevelEventSchedule& schedule,
    ...
    auto const append = [&](simulation::EntityType const type,
    ...
    if (!append(simulation::EntityType::CapitalShip,
                capital_offset,
                capital_end - capital_offset,
                "CapitalShip") ||
        !append(
            simulation::EntityType::Turret, turret_offset, turret_end - turret_offset, "Turret")) {
        return false;
    }
...
auto team(std::string const& id) -> simulation::Team {
    if (id == "red") {
        return simulation::Team::Red;
    }
    if (id == "green") {
        return simulation::Team::Green;
    }
    if (id == "blue") {
        return simulation::Team::Blue;
    }
    if (id == "orange") {
        return simulation::Team::Orange;
    }
    if (id == "yellow") {
        return simulation::Team::Yellow;
    }
    return simulation::Team::White;
}
} // namespace
...
auto compile_level(LevelDefinition const& definition,
                   ...
                   FTurretSimulationConfig const& turret_config) -> LevelCompilationResult {
    ...
    auto const append_entity = [&](std::int32_t const entity_index_value,
        ...
        if (entity.archetype == "capital-ship") {
            ...
            capital_events.locations.set(row,
                                         make_vector3f(static_cast<float>(entity.position.x),
                                                       static_cast<float>(entity.position.y),
                                                       static_cast<float>(entity.position.z)));
            capital_events.rotations.set(
                row,
                simulation::Rotator3f{static_cast<float>(entity.rotation.pitch),
