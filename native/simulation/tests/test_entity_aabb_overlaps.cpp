...
struct OverlapFixture {
    explicit OverlapFixture(ioj::sim::Vector3f const capital_half_extents = {{10.f, 10.f, 10.f}},
                            ioj::sim::Vector3f const capital_centre = ioj::sim::Vector3f{},
                            ioj::sim::Vector3f const turret_half_extents = {{10.f, 10.f, 10.f}})
        : query_manager{registry} {
        auto const type_count{ioj::sim::collision::EntityAABBs::num()};
        for (std::int32_t type_index{}; type_index < type_count; ++type_index) {
            set_bounds(type_index, ioj::sim::Vector3f{}, ioj::sim::Vector3f{{10.f, 10.f, 10.f}});
        }
        set_bounds(ioj::sim::collision::EntityAABBs::capital_ship_index,
                   capital_centre,
                   capital_half_extents);
        set_bounds(ioj::sim::collision::EntityAABBs::static_turret_index,
                   ioj::sim::Vector3f{},
                   turret_half_extents);
    ...
    auto spawn(ioj::sim::Vector3f const location,
               ioj::sim::EntityType const type = ioj::sim::EntityType::CapitalShip,
               ioj::sim::Rotator3f const rotation = ioj::sim::Rotator3f{}) -> RegistryEntityHandle {
    ...
    auto add_static(ioj::sim::Vector3f const min_point, ioj::sim::Vector3f const max_point)
        -> std::int32_t {
    ...
    void run_tick(std::span<RegistryEntityHandle const> const handles,
                  ...
                  std::span<ioj::sim::Rotator3f const> const rotations,
        ...
        for (std::int32_t index{}; index < count; ++index) {
            ...
            if (entity_alive == 0) {
                deaths.add(ioj::sim::DeathReason::Unknown, handle, {});
            }
        }
    ...
    auto get_entity_overlaps() const -> ioj::sim::collision::EntityEntityOverlaps::ConstView {
        return query_manager.get_collision_system()
            .get_aabb_overlap_events()
            .entity_entity_overlaps;
    }

    auto get_static_overlaps() const -> ioj::sim::collision::EntityStaticOverlaps::ConstView {
        return query_manager.get_collision_system()
            .get_aabb_overlap_events()
    ...
    ioj::sim::SpatialQueryManager query_manager;
    ioj::sim::collision::EntityAABBs entity_bounds;
    ioj::sim::SimTick current_tick{};
    bool tick_is_open{};
};

void check_single_pair(ioj::sim::collision::EntityEntityOverlaps::ConstView const overlaps,
                       RegistryEntityHandle const lhs,
                       RegistryEntityHandle const rhs) {
    ioj::sim::tests::expect_equal(overlaps.num(), 1, "Exactly one overlap pair is reported");
    if (overlaps.num() == 1) {
        auto const first{lhs < rhs ? lhs : rhs};
        auto const second{lhs < rhs ? rhs : lhs};
        ioj::sim::tests::expect_true(overlaps.first_entities[0] == first &&
                                         overlaps.second_entities[0] == second,
                                     "Overlap pair is canonical");
        ioj::sim::tests::expect_true(overlaps.first_entities[0] < overlaps.second_entities[0],
                                     "First overlap handle sorts before second");
    }
}

void
    check_single_static_overlap(ioj::sim::collision::EntityStaticOverlaps::ConstView const overlaps,
                                RegistryEntityHandle const entity,
                                std::int32_t const static_geometry_index) {
    ioj::sim::tests::expect_equal(overlaps.num(), 1, "Exactly one static overlap is reported");
    if (overlaps.num() == 1) {
        ioj::sim::tests::expect_true(overlaps.entities[0] == entity &&
...
TEST(EntityAABBOverlaps, MovedEntityOverlapsStationaryEntity) {
    ...
    std::array const locations{ioj::sim::Vector3f{{15.f, 0.f, 0.f}}};
    std::array const rotations{ioj::sim::Rotator3f{}};
...
TEST(EntityAABBOverlaps, TwoMovedEntitiesProduceOnePair) {
    ...
    std::array const locations{ioj::sim::Vector3f{{-5.f, 0.f, 0.f}},
                               ioj::sim::Vector3f{{5.f, 0.f, 0.f}}};
    std::array const rotations{ioj::sim::Rotator3f{}, ioj::sim::Rotator3f{}};
    fixture.run_tick(handles, locations, rotations);
...
TEST(EntityAABBOverlaps, SharedCellWithoutExactOverlapProducesNoPair) {
    ...
    std::array const handles{moved};
    std::array const locations{ioj::sim::Vector3f{{10.f, 10.f, 10.f}}};
    std::array const rotations{ioj::sim::Rotator3f{}};
    fixture.run_tick(handles, locations, rotations);

    ioj::sim::tests::expect_equal(fixture.get_entity_overlaps().num(),
                                  0,
    ...
    auto const cell_entities{
        fixture.query_manager.get_collision_system().get_uniform_grid().get_cell_entities(
            {20, 20, 20})};
    ioj::sim::tests::expect_true(std::ranges::find(cell_entities, stationary) !=
                                         cell_entities.end() &&
...
TEST(EntityAABBOverlaps, MultiCellOverlapProducesOnePair) {
    ...
    std::array const handles{moved};
    std::array const locations{ioj::sim::Vector3f{{10.f, 0.f, 0.f}}};
    std::array const rotations{ioj::sim::Rotator3f{}};
    fixture.run_tick(handles, locations, rotations);
...
TEST(EntityAABBOverlaps, SelfAndQuietTicksProduceNoPairs) {
    ...
    std::array const locations{ioj::sim::Vector3f{{1.f, 0.f, 0.f}}};
    std::array const rotations{ioj::sim::Rotator3f{}};
    fixture.run_tick(handles, locations, rotations);
    ioj::sim::tests::expect_equal(
...
TEST(EntityAABBOverlaps, RotatedConservativeWorldBoundsUseExistingBoundsRules) {
    ...
    auto const stationary{fixture.spawn({{0.f, 60.f, 0.f}}, ioj::sim::EntityType::Turret)};
    fixture.finish_spawning();

    auto const rotation{ioj::sim::Rotator3f{0.f, 90.f, 0.f}};
    auto const expected_bounds{ioj::sim::collision::make_entity_world_bounds(
        fixture.entity_bounds,
        ioj::sim::collision::EntityAABBs::capital_ship_index,
        ioj::sim::Vector3f{},
        ioj::sim::to_quaternion(rotation))};
    ioj::sim::tests::expect_true(expected_bounds.min.Y <= 55.f && expected_bounds.max.Y >= 65.f,
                                 "Rotated conservative bounds reach the stationary entity");

    std::array const handles{rotated};
    std::array const locations{ioj::sim::Vector3f{}};
...
TEST(EntityAABBOverlaps, MovingOutOfOverlapRemovesPairFromCurrentResult) {
    ...
    std::array const handles{moved};
    std::array const rotations{ioj::sim::Rotator3f{}};
    std::array const overlapping_location{ioj::sim::Vector3f{{15.f, 0.f, 0.f}}};
    fixture.run_tick(handles, overlapping_location, rotations);
    check_single_pair(fixture.get_entity_overlaps(), moved, stationary);

    fixture.end_tick();
    std::array const separated_location{ioj::sim::Vector3f{{100.f, 0.f, 0.f}}};
...
TEST(EntityAABBOverlaps, MovedEntityOverlapsMultipleStaticAABBs) {
    ...
    ioj::sim::tests::expect_equal(overlaps.num(), 2, "Both static overlaps are reported");
    if (overlaps.num() == 2) {
        ioj::sim::tests::expect_true(overlaps.entities[0] == moved &&
...
TEST(EntityAABBOverlaps, SharedCellWithoutExactStaticOverlapProducesNoRecord) {
    ...
    ioj::sim::tests::expect_equal(fixture.get_static_overlaps().num(),
                                  0,
...
TEST(EntityAABBOverlaps, MultipleMovedEntitiesProduceDistinctStaticRecords) {
    ...
    if (overlaps.num() == 2) {
        ioj::sim::tests::expect_true(overlaps.entities[0] == first &&
...
TEST(EntityAABBOverlaps, RotationOnlyMovementUsesConservativeBoundsForBothStreams) {
    ...
    std::array const handles{rotated};
    std::array const locations{ioj::sim::Vector3f{}};
    std::array const rotations{ioj::sim::Rotator3f{0.f, 90.f, 0.f}};
    fixture.run_tick(handles, locations, rotations);
...
TEST(EntityAABBOverlaps, ManyMovedEntitiesProduceSortedUniqueResults) {
    ...
    std::vector<ioj::sim::Vector3f> moved_locations{};
    std::vector<ioj::sim::Rotator3f> moved_rotations{};
    ...
    for (std::int32_t index{}; index < entity_overlaps.num(); ++index) {
        ...
        ioj::sim::tests::expect_true(entity_overlaps.first_entities[index] <
    ...
    ioj::sim::tests::expect_true(static_overlaps.num() >= entity_count,
    ...
    for (std::int32_t index{}; index < static_overlaps.num(); ++index) {
        ...
        auto const static_geometry_count{fixture.query_manager.get_collision_system()
                                             ...
                                             .get_static_aabbs()
                                             .num()};
        ioj::sim::tests::expect_true(static_overlaps.static_geometry_indices[index] >= 0 &&
    ...
    for (auto const moved : moved_entities) {
        ...
        ioj::sim::tests::expect_true(found_large_static,
...
TEST(EntityAABBOverlaps, EventsMirrorAuthoritativeResults) {
    ...
    ioj::sim::tests::expect_equal(static_cast<std::int32_t>(events.batches.size()),
                                  1,
...
TEST(EntityAABBOverlaps, FrameEventResetRetainsStorageAndPassesAppend) {
    ...
    ioj::sim::tests::expect_equal(static_cast<std::int32_t>(reset_events.batches.size()),
                                  0,
                                  "Frame reset clears event batches");
    ioj::sim::tests::expect_true(reset_events.entity_entity_overlaps.first_entities.data() ==
                                     entity_storage,
                                 "Dynamic event storage is retained across reset");
    ioj::sim::tests::expect_true(reset_events.entity_static_overlaps.entities.data() ==
                                     static_storage,
    ...
    ioj::sim::tests::expect_true(recaptured_events.entity_entity_overlaps.first_entities.data() ==
                                     entity_storage,
                                 "Dynamic event storage is reused after recapture");
    ioj::sim::tests::expect_true(recaptured_events.entity_static_overlaps.entities.data() ==
                                     static_storage,
    ...
    ioj::sim::tests::expect_equal(appended_events.entity_entity_overlaps.num(),
                                  2,
                                  "Collision passes append dynamic events within a frame");
    ioj::sim::tests::expect_equal(appended_events.entity_static_overlaps.num(),
                                  2,
                                  "Collision passes append static events within a frame");
    ioj::sim::tests::expect_equal(static_cast<std::int32_t>(appended_events.batches.size()),
                                  2,
                                  "Collision passes retain separate batch metadata");
    if (appended_events.batches.size() == 2) {
        ioj::sim::tests::expect_true(appended_events.batches[0].tick <
    ...
    ioj::sim::tests::expect_equal(next_frame_events.entity_entity_overlaps.num(),
                                  0,
                                  "The next frame starts without dynamic events");
    ioj::sim::tests::expect_equal(next_frame_events.entity_static_overlaps.num(),
                                  0,
                                  "The next frame starts without static events");
    ioj::sim::tests::expect_equal(static_cast<std::int32_t>(next_frame_events.batches.size()),
                                  0,
...
TEST(EntityAABBOverlaps, InvalidDeadAndStaleDirtyHandlesAreIgnored) {
    ...
    std::array const removed_location{ioj::sim::Vector3f{{10.f, 0.f, 0.f}}};
    std::array const rotations{ioj::sim::Rotator3f{}};
    std::array const dead{std::uint8_t{0}};
    fixture.run_tick(removed_handle, removed_location, rotations, dead);
    ioj::sim::tests::expect_true(fixture.registry.is_valid_dead(removed),
    ...
    ioj::sim::tests::expect_true(fixture.registry.is_stale(removed),
