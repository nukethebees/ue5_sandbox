...
TEST(NativeSimulation, FFighterEntityBiasPackedDataTest) {
    ...
    ml::simulation_tests::expect_equal(view.integral_biases[0], 200u, "Mutable view integral bias");
    ml::simulation_tests::expect_equal(view.float_biases[1], 0.3f, "Mutable view float bias");
    ...
    ml::simulation_tests::expect_equal(
        const_view.integral_biases[1], 300u, "Const view integral bias");
    ml::simulation_tests::expect_equal(const_view.float_biases[0], 0.2f, "Const view float bias");
    ...
    ml::simulation_tests::expect_true(reordered.entity_handles[0] == FRegistryEntityHandle{30, 3},
                                      "Buffered copy keeps handle paired with integral bias");
    ml::simulation_tests::expect_equal(
        reordered.integral_biases[0], 300u, "Buffered copy integral bias");
    ml::simulation_tests::expect_equal(reordered.float_biases[0], 0.3f, "Buffered copy float bias");

    reordered.remove_at_swap(0, 1);
    reordered.validate_array_sizes();
    ml::simulation_tests::expect_true(reordered.entity_handles[0] == FRegistryEntityHandle{20, 2},
                                      "Swap removal keeps handle paired with integral bias");
    ml::simulation_tests::expect_equal(
        reordered.integral_biases[0], 200u, "Swap removal integral bias");
    ml::simulation_tests::expect_equal(reordered.float_biases[0], 0.2f, "Swap removal float bias");
