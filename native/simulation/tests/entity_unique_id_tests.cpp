...
namespace {
static_assert(EntityUniqueId::index_field::offset == 0);
static_assert(EntityUniqueId::index_field::bits == 24);
static_assert(EntityUniqueId::index_field::value_mask == 0x00ffffffu);
static_assert(EntityUniqueId::index_field::mask == 0x00ffffffu);
static_assert(EntityUniqueId::entity_type_field::offset == 24);
static_assert(EntityUniqueId::entity_type_field::bits == 8);
static_assert(EntityUniqueId::entity_type_field::value_mask == 0x000000ffu);
static_assert(EntityUniqueId::entity_type_field::mask == 0xff000000u);
static_assert((EntityUniqueId::index_field::mask & EntityUniqueId::entity_type_field::mask) == 0);
static_assert((EntityUniqueId::index_field::mask | EntityUniqueId::entity_type_field::mask) ==
              0xffffffffu);
...
constexpr auto zero_player_id{EntityUniqueId(0, EntityType::PlayerShip)};
...
constexpr auto maximum_fighter_id{
    EntityUniqueId(EntityUniqueId::index_field::value_mask, EntityType::Fighter)};
static_assert(maximum_fighter_id.index() == EntityUniqueId::index_field::value_mask);
...
TEST(EntityUniqueId, RoundTripsRepresentativeIndicesAndEntityTypes) {
    ...
    for (auto const index : indices) {
        for (auto const type : types) {
            auto const id{EntityUniqueId(index, type)};
...
TEST(EntityUniqueId, IndexAndTypeBitsDoNotOverlap) {
    ...
    auto const turret{EntityUniqueId(index, EntityType::Turret)};
    auto const fighter{EntityUniqueId(index, EntityType::Fighter)};
    EXPECT_EQ(turret.index(), fighter.index());
    EXPECT_NE(turret.raw_value(), fighter.raw_value());

    auto const first{EntityUniqueId(1, EntityType::CapitalShip)};
    auto const second{EntityUniqueId(0x00fedcba, EntityType::CapitalShip)};
...
TEST(EntityUniqueId, PacksFieldsDirectly) {
    auto const id{EntityUniqueId(0x00123456, EntityType::CapitalShip)};
...
TEST(EntityUniqueId, InvalidRepresentationAndDomainArePreserved) {
    ...
    EXPECT_FALSE(EntityUniqueId::from_raw(std::uint32_t{0xffffffff}).is_valid());
}
