...
void run_worldless_entity_registry_scenario(ml::simulation_tests::SimulationFixture const& config,
    ...
    if (scenario != EEntityRegistryScenario::TeamCounts) {
        data.player.emplace(ml::simulation_tests::make_player_spawn(config));
    }
    ...
    for (std::int32_t team_index{};
        ...
        for (std::int32_t i{}; i < expected_team_counts[team_index]; ++i) {
            ...
            data.capital_spawns.teams[index] =
                static_cast<ml::simulation::Team>(static_cast<ml::simulation::Team>(team_index));
    ...
    if (scenario == EEntityRegistryScenario::TeamCounts) {
        ...
        ml::simulation_tests::expect_equal(15, total_count, "Check entity total");
        for (std::int32_t team_index{};
             team_index < static_cast<std::int32_t>(expected_team_counts.size());
             ++team_index) {
            auto const team{static_cast<ml::simulation::Team>(team_index)};
            ml::simulation_tests::expect_equal(expected_team_counts[team_index],
                                               counts[team_index],
                                               "Count team " + ::testing::PrintToString(team));
            std::int32_t type_count{};
            constexpr auto type_count_limit{std::to_underlying(ml::simulation::EntityType::COUNT)};
            for (std::int32_t type{}; type < type_count_limit; ++type) {
                type_count += type_counts[team_index][type];
            }
            ml::simulation_tests::expect_equal(counts[team_index],
                                               type_count,
    ...
    ml::simulation_tests::expect_greater(static_cast<std::int32_t>(available_targets.size()),
                                         expected_kills - 1,
    ...
    ml::simulation_tests::expect_true(!samples.is_empty(), "Kill samples recorded");
    if (samples.is_empty()) {
        return;
    }
    ...
    ml::simulation_tests::expect_equal(
        0, before.player_kills, "Player kills are zero before event");
    ml::simulation_tests::expect_equal(0, before.total_kills, "Total kills are zero before event");
    ml::simulation_tests::expect_equal(
    ...
    ml::simulation_tests::expect_equal(initial_alive_count - expected_kills,
                                       after.alive_count,
