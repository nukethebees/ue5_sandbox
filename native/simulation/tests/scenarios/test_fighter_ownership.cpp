...
void run_worldless_simultaneous_capital_reassignment(tests::SimulationFixture const& config) {
    ...
    harness.timeline.at(1.0, [&] {
        ...
        for (auto const team : {Team::Green, Team::Red}) {
            ...
            EXPECT_TRUE(victim.is_valid()) << "Team has a capital to kill";
            if (victim.is_valid()) {
    ...
    EXPECT_TRUE(harness.run_until_timeline_finished(10.0))
        << "Capital reassignment timeline completes";
    EXPECT_TRUE(samples.num() > 1) << "Capital reassignment samples are recorded";
    ...
    EXPECT_EQ(4, static_cast<std::int32_t>(before.capitals.size()))
        << "Two capitals per team spawn";
    EXPECT_EQ(2, static_cast<std::int32_t>(after.capitals.size()))
        << "Both killed capitals are removed";
    EXPECT_GT(static_cast<std::int32_t>(before.fighters.size()), 0) << "One fighter wave spawns";
    EXPECT_EQ(static_cast<std::int32_t>(before.fighters.size()),
    ...
    for (std::int32_t i{}; i < count; ++i) {
        EXPECT_TRUE(!std::ranges::contains(killed_capitals, after.capitals[i]))
            << "Owner capital survives";
        EXPECT_EQ(offset, after.span_starts[i]) << "Ownership spans are contiguous";
        ...
        EXPECT_EQ(expected_count, static_cast<std::int32_t>(after.owned_fighters[i].size()))
        ...
        for (auto const fighter : after.owned_fighters[i]) {
            ...
            EXPECT_EQ(after.capital_teams[i], after.fighter_teams[offset])
                << "Fighter belongs to owner's team";
            EXPECT_EQ(after.capitals[i], after.fighter_parents[offset])
            ...
            if (offset >= 0 && static_cast<std::size_t>(offset) < after.all_owned_fighters.size()) {
                EXPECT_EQ(fighter, after.all_owned_fighters[offset])
                    << "Span matches flat ownership array";
            }
            ++offset;
        }
    }
    EXPECT_EQ(static_cast<std::int32_t>(after.fighters.size()),
              static_cast<std::int32_t>(seen.size()))
        << "Every fighter has one owner";
    EXPECT_EQ(offset, static_cast<std::int32_t>(after.all_owned_fighters.size()))
...
void run_worldless_fighter_ownership(tests::SimulationFixture const& config,
    ...
    harness.timeline.at(0.2, [&] {
        EXPECT_EQ(3, capitals.get_num_instances()) << "Three capitals are registered";
        auto const expected_fighters{3 * capitals.get_fighter_spawn_slots()};
        EXPECT_EQ(expected_fighters, fighters.get_num_instances())
        ...
        for (auto const target : fighters.get_target_ids()) {
            EXPECT_TRUE(target.is_valid()) << "Spawned fighter has a target";
        }
    ...
    if (scenario != FighterOwnershipScenario::KillCapital) {
        ...
        next_time += 0.2;
        harness.timeline.at(next_time, [&] {
            EXPECT_EQ(static_cast<std::int32_t>(kept.size()), fighters.get_num_instances())
                << "Killed fighters are removed from the simulation";
            EXPECT_EQ(static_cast<std::int32_t>(kept.size()),
    ...
    if (scenario != FighterOwnershipScenario::KillFightersOnly) {
        ...
        next_time += 0.5;
        harness.timeline.at(next_time, [&] {
            EXPECT_EQ(2, capitals.get_num_instances())
                << "Killed capital is removed from the simulation";
            auto const main_index{capitals.find_first_index_on_team(Team::Green)};
            EXPECT_TRUE(main_index.has_value()) << "Green capital survives";
            if (main_index.has_value()) {
                auto const remaining{capitals.get_fighter_ids(*main_index)};
                EXPECT_EQ(static_cast<std::int32_t>(green_fighters_before_capital_kill.size()),
    ...
    EXPECT_TRUE(harness.run_until_timeline_finished(next_time + 0.2))
        << "Capital fighter ID timeline completes";
    EXPECT_TRUE(initial_checked) << "Initial fighter ownership is checked";
    EXPECT_TRUE(fighter_kill_checked) << "Fighter removal is checked";
    EXPECT_TRUE(capital_kill_checked) << "Capital removal is checked";
}
}
