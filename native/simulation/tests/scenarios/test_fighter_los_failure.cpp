...
namespace ml {
void run_worldless_fighter_los_failure(ml::simulation_tests::SimulationFixture const& config) {
    ...
    std::vector<std::vector<ml::simulation::Team>> fighter_team_samples{};
    harness.on_end_tick = [&](FLevelSimulation&) {
        std::vector<ml::simulation::Team> teams{};
    ...
    ml::simulation_tests::expect_true(harness.run_until_timeline_finished(31.0),
                                      "Fighter line-of-sight failure timeline completes");
    ml::simulation_tests::expect_true(!fighter_team_samples.empty(), "Simulation produced samples");
    for (auto const& teams : fighter_team_samples) {
        ml::simulation_tests::expect_true(
            std::ranges::all_of(teams,
                                [](auto team) { return team == ml::simulation::Team::Blue; }),
