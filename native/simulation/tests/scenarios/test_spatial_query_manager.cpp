...
namespace ml {
void run_worldless_spatial_query_line_of_sight(
    ml::simulation_tests::SimulationFixture const& config) {
    constexpr float distance{30000.f};
    std::vector<ml::simulation::Vector3f> const locations{{{0.f, distance, 0.f}},
                                                          {{0.f, -distance, 0.f}},
                                                          {{distance, 0.f, 0.f}},
                                                          {{-distance, 0.f, 0.f}}};
    ...
    for (auto const location : locations) {
        ml::simulation_tests::add_capital_spawn(
            data, location, ml::simulation::Team::White, -1, 999.f, 999.f);
    }
    ...
    for (std::int32_t i{}; i < count; ++i) {
        ml::simulation_tests::expect_true(results[i].is_null(), "Half-distance trace misses", i);
    ...
    for (std::int32_t i{}; i < count; ++i) {
        ml::simulation_tests::expect_equal(
            std::uint8_t{1}, has_los[i], "Clear line remains visible", i);
        ml::simulation_tests::expect_equal(
    ...
    ml::simulation_tests::expect_true(true, "Line-of-sight query batch completed");
}
