...
namespace ml {
namespace {
ml::simulation::collision::CellCoord const grid_dims{400, 400, 5};
ml::simulation::Vector3f const cell_dims{{5000.f, 5000.f, 20000.f}};
ml::simulation::collision::CellCoord const trace_grid_dims{8, 8, 8};
ml::simulation::Vector3f const trace_cell_dims{{100.f, 100.f, 100.f}};
constexpr float hit_location_tolerance{0.001f};

struct FTraceFixture {
    FTraceFixture(std::span<ml::simulation::Vector3f const> const locations,
                  ml::simulation::Vector3f const half_extents,
                  ml::simulation::Vector3f const aabb_centre = ml::simulation::Vector3f{},
                  ml::simulation::collision::CellCoord const fixture_grid_dims = trace_grid_dims,
                  ml::simulation::Vector3f const fixture_cell_dims = trace_cell_dims,
                  std::span<ml::simulation::EntityType const> const fixture_entity_types = {},
        ...
        handles = ml::simulation_tests::copy_handles(spawned.registry_handles);

        set_entity_aabb(ml::simulation::EntityType::CapitalShip, aabb_centre, half_extents);
    ...
    void set_entity_aabb(ml::simulation::EntityType const entity_type,
    ...
    void update_entities(std::span<ml::simulation::Vector3f const> const locations,
    ...
    auto add_entity(ml::simulation::Vector3f const location) -> FRegistryEntityHandle {
        simulation::RegistryEntityData entity_data;
    ...
    ml::simulation::collision::EntityAABBs aabbs;
};

auto reference_trace_aabb(ml::simulation::Vector3f const start,
                          ml::simulation::Vector3f const end,
                          ml::simulation::Vector3f const aabb_min,
                          ml::simulation::Vector3f const aabb_max) -> float {
...
auto make_line_traces(std::span<ml::simulation::Vector3f const> const starts,
                      std::span<ml::simulation::Vector3f const> const ends)
    -> ml::simulation::LineTraces {
    assert(static_cast<std::int32_t>(starts.size()) == static_cast<std::int32_t>(ends.size()));

    ml::simulation::LineTraces traces;
...
auto run_traces(FTraceFixture const& fixture,
                std::span<ml::simulation::Vector3f const> const starts,
                std::span<ml::simulation::Vector3f const> const ends,
                std::span<FRegistryEntityHandle const> const ignored_entities = {}) -> FTraceHits {
...
struct FExpectedTrace {
    char const* name;
    ml::simulation::Vector3f start;
    ml::simulation::Vector3f end;
    std::uint8_t expected_hit;
    ml::simulation::Vector3f expected_location{};
    std::int32_t expected_entity_index{};
};

void check_traces(FTraceFixture const& fixture, std::span<FExpectedTrace const> const cases) {
    std::vector<ml::simulation::Vector3f> starts{};
    std::vector<ml::simulation::Vector3f> ends{};
    ...
    for (std::int32_t i{}; i < count; ++i) {
        ...
        ml::simulation_tests::expect_equal(trace_case.expected_hit, hits.hits[i], hit_description);
        ...
        std::string const entity_description{" resolves expected entity" +
                                             ::testing::PrintToString(trace_case.name)};
        ml::simulation_tests::expect_equal(fixture.handles[trace_case.expected_entity_index],
                                           hits.entities[i],
                                           entity_description);

        std::string const location_description{" resolves expected hit location" +
                                               ::testing::PrintToString(trace_case.name)};
        ml::simulation_tests::expect_distance_near(trace_case.expected_location,
                                                   hits.locations[i],
...
auto count_handle(std::span<FRegistryEntityHandle const> const handles,
                  FRegistryEntityHandle const expected) -> std::int32_t {
    std::int32_t count{};
...
void run_worldless_collision_uniform_grid_membership(
    ...
    data.player = ml::simulation_tests::make_player_spawn(
        config,
        ml::simulation::Transform3d{.rotation = ml::Quaternion4d{},
                                    .location = ml::Vector3d{-1500.f, -1500.f, 0.f}});
    ml::simulation_tests::add_capital_spawn(data,
                                            ml::simulation::Vector3f{{-500.f, -500.f, 0.f}},
    ...
    ml::simulation_tests::expect_not_null(player, "Collision-grid player ship is available");
    ml::simulation_tests::expect_true(!fighter_handles.empty(), "Collision-grid fighter is placed");
    if (::testing::Test::HasFailure()) {
        return;
    }

    std::array<FRegistryEntityHandle, 5> const expected_handles{
    ...
    for (std::int32_t i{}; i < handle_count; ++i) {
        ...
        auto const entity_type{static_cast<ml::simulation::EntityType>(i)};
        if (!ml::simulation_tests::expect_true(registry.is_valid_alive(handle),
        ...
        if (!ml::simulation_tests::expect_true(grid.is_cell_coord_in_bounds(min_coord, max_coord),
...
class FCollisionUniformGridTraceScenario final {
  public:
    FCollisionUniformGridTraceScenario(ECollisionUniformGridTraceScenario scenario);
    void run();
  private:
...
void FCollisionUniformGridTraceScenario::test_hits_and_misses() {
    ml::simulation::Vector3f const entity_centre{};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    ml::simulation::Vector3f const left_of_aabb{{-150.f, 0.f, 0.f}};
    ml::simulation::Vector3f const right_of_aabb{{150.f, 0.f, 0.f}};
    ml::simulation::Vector3f const negative_diagonal_start{{-150.f, -150.f, -150.f}};
    ml::simulation::Vector3f const positive_diagonal_end{{150.f, 150.f, 150.f}};
    ml::simulation::Vector3f const off_axis_start{{-150.f, 25.f, 0.f}};
    ml::simulation::Vector3f const off_axis_end{{150.f, 25.f, 0.f}};
    ml::simulation::Vector3f const left_face_contact{{-10.f, 0.f, 0.f}};
    ml::simulation::Vector3f const right_face_contact{{10.f, 0.f, 0.f}};
    ml::simulation::Vector3f const negative_corner_contact{{-10.f, -10.f, -10.f}};
    ml::simulation::Vector3f const unused_miss_location{};

    std::vector<ml::simulation::Vector3f> const entity_locations{entity_centre};
    FTraceFixture const fixture{entity_locations, aabb_half_extents};

    std::vector<ml::simulation::Vector3f> const starts{
        left_of_aabb,
        right_of_aabb,
    ...
    std::vector<ml::simulation::Vector3f> const ends{
        right_of_aabb,
        left_of_aabb,
    ...
    std::array<ml::simulation::Vector3f, 5> const expected_locations{
        left_face_contact,
        right_face_contact,
    ...
    for (std::int32_t i{}; i < count; ++i) {
        ...
        ml::simulation_tests::expect_distance_near(expected_locations[i],
                                                   hits.locations[i],
                                                   hit_location_tolerance,
...
void FCollisionUniformGridTraceScenario::test_stops_at_endpoint() {
    ml::simulation::Vector3f const entity_centre{{50.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    ml::simulation::Vector3f const trace_start{};
    ml::simulation::Vector3f const trace_endpoint_before_aabb{{20.f, 0.f, 0.f}};

    std::vector<ml::simulation::Vector3f> const entity_locations{entity_centre};
    FTraceFixture const fixture{entity_locations, aabb_half_extents};
    std::vector<ml::simulation::Vector3f> const starts{trace_start};
    std::vector<ml::simulation::Vector3f> const ends{trace_endpoint_before_aabb};
...
void FCollisionUniformGridTraceScenario::test_returns_nearest_hit() {
    ml::simulation::Vector3f const far_entity_centre{{60.f, 0.f, 0.f}};
    ml::simulation::Vector3f const near_entity_centre{{20.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{5.f, 5.f, 5.f}};
    ml::simulation::Vector3f const trace_start{};
    ml::simulation::Vector3f const trace_end{{90.f, 0.f, 0.f}};
    ml::simulation::Vector3f const expected_near_contact{{15.f, 0.f, 0.f}};

    std::vector<ml::simulation::Vector3f> const entity_locations{
        far_entity_centre,
        near_entity_centre,
    };
    FTraceFixture const fixture{entity_locations, aabb_half_extents};
    std::vector<ml::simulation::Vector3f> const starts{trace_start};
    std::vector<ml::simulation::Vector3f> const ends{trace_end};
    ...
    ml::simulation_tests::expect_distance_near(expected_near_contact,
                                               hits.locations[0],
                                               hit_location_tolerance,
...
void FCollisionUniformGridTraceScenario::test_handles_zero_length_traces() {
    ml::simulation::Vector3f const entity_centre{{50.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    ml::simulation::Vector3f const point_outside_aabb{};
    ...
    ml::simulation_tests::expect_distance_near(starts[0],
                                               hits.locations[0],
                                               hit_location_tolerance,
...
void FCollisionUniformGridTraceScenario::test_includes_negative_endpoint_boundary() {
    ml::simulation::Vector3f const entity_centre{{-10.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    ml::simulation::Vector3f const trace_start{{10.f, 0.f, 0.f}};
    ml::simulation::Vector3f const boundary_contact{};

    std::vector<ml::simulation::Vector3f> const entity_locations{entity_centre};
    FTraceFixture const fixture{entity_locations, aabb_half_extents};
    std::vector<ml::simulation::Vector3f> const starts{trace_start};
    std::vector<ml::simulation::Vector3f> const ends{boundary_contact};
    ...
    ml::simulation_tests::expect_distance_near(boundary_contact,
                                               hits.locations[0],
                                               hit_location_tolerance,
...
void FCollisionUniformGridTraceScenario::test_applies_aabb_centre() {
    ...
    FTraceFixture rotated{std::vector<ml::simulation::Vector3f>{ml::simulation::Vector3f{}},
                          ml::simulation::Vector3f{{20.f, 5.f, 10.f}},
                          ml::simulation::Vector3f{{150.f, 0.f, 0.f}},
                          trace_grid_dims,
                          trace_cell_dims,
                          {},
                          ml::simulation::Rotator3f{0.f, 90.f, 0.f}};
    std::vector<ml::simulation::Vector3f> const rotated_starts{{{-50.f, 150.f, 0.f}},
                                                               {{100.f, 0.f, 0.f}}};
    std::vector<ml::simulation::Vector3f> const rotated_ends{{{50.f, 150.f, 0.f}},
                                                             {{200.f, 0.f, 0.f}}};
    ...
    ml::simulation_tests::expect_distance_near(ml::simulation::Vector3f{{-5.f, 150.f, 0.f}},
                                               rotated_hits.locations[0],
                                               0.001f,
                                               "Rotated box has swapped extents");
    auto const swept{run_sweeps(
        rotated, rotated_starts, rotated_ends, ml::simulation::Vector3f{{2.f, 2.f, 2.f}})};
    ml::simulation_tests::expect_equal(
        std::uint8_t{1}, swept.hits[0], "Sweep uses rotated cached box");
    ml::simulation_tests::expect_distance_near(ml::simulation::Vector3f{{-7.f, 150.f, 0.f}},
                                               swept.locations[0],
                                               0.001f,
    ...
    ml::simulation_tests::expect_distance_near(
        ml::simulation::Vector3f{{-5.f, 130.f, -10.f}},
        simulation::collision::min_at(cached, 0),
        0.001f,
        "Visualisation reads the same rotated cached bounds");

    rotated.update_entities(std::vector<ml::simulation::Vector3f>{ml::simulation::Vector3f{}},
                            std::vector<std::uint8_t>{1});
    auto const updated_hits{run_traces(rotated, rotated_starts, rotated_ends)};
    ml::simulation_tests::expect_equal(std::uint8_t{0},
                                       updated_hits.hits[0],
    ...
    auto const reused_handle{rotated.add_entity(ml::simulation::Vector3f{})};
    ml::simulation_tests::expect_equal(
    ...
    ml::simulation::Vector3f const entity_location{};
    ml::simulation::Vector3f const local_aabb_centre{{40.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    ml::simulation::Vector3f const trace_start{{20.f, 0.f, 0.f}};
    ml::simulation::Vector3f const trace_end{{60.f, 0.f, 0.f}};
    ml::simulation::Vector3f const expected_contact{{30.f, 0.f, 0.f}};

    std::vector<ml::simulation::Vector3f> const entity_locations{entity_location};
    ...
    ml::simulation_tests::expect_equal(
        std::uint8_t{1}, hits.hits[0], "Trace hits locally centred AABB");
    if (hits.hits[0] == 0) {
        return;
    }
    ...
    ml::simulation_tests::expect_distance_near(expected_contact,
                                               hits.locations[0],
                                               hit_location_tolerance,
                                               "Trace applies local AABB centre to hit location");

    std::vector<ml::simulation::EntityType> const entity_types{
        ml::simulation::EntityType::PlayerShip,
        ml::simulation::EntityType::Turret,
        ml::simulation::EntityType::CapitalShip,
        ml::simulation::EntityType::CapitalShipFighter,
        ml::simulation::EntityType::TubeSpinner,
    };
    std::vector<ml::simulation::Vector3f> const mixed_locations{
        {{-300.f, 0.f, 0.f}},
        {{-150.f, 0.f, 0.f}},
        {{0.f, 0.f, 0.f}},
        {{150.f, 0.f, 0.f}},
    ...
    std::vector<ml::simulation::Vector3f> const local_centres{
        {{5.f, -20.f, 3.f}},
        {{-6.f, -10.f, -2.f}},
        {{0.f, 0.f, 0.f}},
        {{8.f, 10.f, -4.f}},
    ...
    std::vector<ml::simulation::Vector3f> const half_extents{
        {{4.f, 5.f, 6.f}},
        {{7.f, 8.f, 9.f}},
        {{10.f, 11.f, 12.f}},
        {{13.f, 14.f, 15.f}},
    ...
    FTraceFixture mixed_fixture{
        mixed_locations,
        half_extents[ml::simulation::collision::EntityAABBs::capital_ship_index],
        local_centres[ml::simulation::collision::EntityAABBs::capital_ship_index],
        trace_grid_dims,
        trace_cell_dims,
        entity_types};
    ...
    std::vector<ml::simulation::Vector3f> mixed_starts{};
    std::vector<ml::simulation::Vector3f> mixed_ends{};
    ...
    for (std::int32_t i{}; i < entity_type_count; ++i) {
        ...
        mixed_starts.push_back(world_centre - ml::simulation::Vector3f{{0.f, 50.f, 0.f}});
        mixed_ends.push_back(world_centre + ml::simulation::Vector3f{{0.f, 50.f, 0.f}});
    }
    ...
    for (std::int32_t i{}; i < entity_type_count; ++i) {
        ...
        ml::simulation_tests::expect_equal(mixed_fixture.handles[i],
                                           mixed_hits.entities[i],
                                           "Mixed entity-type trace resolves its entity",
                                           i);
        auto const world_centre{mixed_locations[i] + local_centres[i]};
        auto const expected_type_contact{world_centre -
                                         ml::simulation::Vector3f{{0.f, half_extents[i].Y, 0.f}}};
        ml::simulation_tests::expect_distance_near(expected_type_contact,
...
void FCollisionUniformGridTraceScenario::test_axis_parallel_and_origin() {
    ...
    std::vector<ml::simulation::Vector3f> const entity_locations{entity_centre};
    FTraceFixture const fixture{entity_locations, aabb_half_extents};
    std::vector<FExpectedTrace> const cases{
...
void FCollisionUniformGridTraceScenario::test_grid_boundary_traversal() {
    ...
    {
        std::vector<ml::simulation::Vector3f> const entity_locations{{{-10.f, 0.f, 0.f}}};
        FTraceFixture const fixture{entity_locations, aabb_half_extents};
        std::vector<FExpectedTrace> const cases{
    ...
    {
        std::vector<ml::simulation::Vector3f> const entity_locations{{{0.f, -10.f, 0.f}}};
        FTraceFixture const fixture{entity_locations, aabb_half_extents};
        std::vector<FExpectedTrace> const cases{
    ...
    {
        std::vector<ml::simulation::Vector3f> const entity_locations{{{0.f, 0.f, -10.f}}};
        FTraceFixture const fixture{entity_locations, aabb_half_extents};
        std::vector<FExpectedTrace> const cases{
    ...
    {
        std::vector<ml::simulation::Vector3f> const entity_locations{{{10.f, -10.f, 0.f}}};
        FTraceFixture const fixture{entity_locations, aabb_half_extents};
        std::vector<FExpectedTrace> const cases{
    ...
    {
        std::vector<ml::simulation::Vector3f> const entity_locations{{{10.f, -10.f, -10.f}}};
        FTraceFixture const fixture{entity_locations, aabb_half_extents};
        std::vector<FExpectedTrace> const cases{
...
void FCollisionUniformGridTraceScenario::test_degenerate_aabbs() {
    ml::simulation::Vector3f const cell_interior{{25.f, 25.f, 25.f}};
    constexpr float trace_offset{25.f};

    {
        ml::simulation::Vector3f const point_half_extents{};
        std::vector<ml::simulation::Vector3f> const entity_locations{cell_interior};
    ...
    {
        ml::simulation::Vector3f const plane_half_extents{{0.f, 10.f, 10.f}};
        std::vector<ml::simulation::Vector3f> const entity_locations{cell_interior};
    ...
    {
        ml::simulation::Vector3f const line_half_extents{{10.f, 0.f, 0.f}};
        std::vector<ml::simulation::Vector3f> const entity_locations{cell_interior};
...

    {
        ...
        ml::simulation::Vector3f const point_half_extents{};
        std::vector<ml::simulation::Vector3f> const entity_locations{origin};
        FTraceFixture const fixture{entity_locations, point_half_extents};
        std::vector<FExpectedTrace> const cases{
...
void FCollisionUniformGridTraceScenario::test_cross_cell_nearest_hit() {
    ml::simulation::Vector3f const positive_entity_centre{{150.f, 0.f, 0.f}};
    ml::simulation::Vector3f const negative_entity_centre{{-150.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    ...
    {
        ml::simulation::Vector3f const wide_aabb_half_extents{{160.f, 10.f, 10.f}};
        std::vector<ml::simulation::Vector3f> const entity_locations{{{0.f, 0.f, 0.f}}};
...
void FCollisionUniformGridTraceScenario::test_varied_grid_geometry() {
    ml::simulation::collision::CellCoord const fixture_grid_dims{5, 7, 3};
    ml::simulation::Vector3f const fixture_cell_dims{{80.f, 125.f, 250.f}};
    ml::simulation::Vector3f const entity_centre{{25.f, -30.f, 40.f}};
    ml::simulation::Vector3f const aabb_half_extents{{15.f, 20.f, 25.f}};
    ...
    ml::simulation::Vector3f const boundary_entity_centre{{-55.f, -30.f, 40.f}};
    std::vector<ml::simulation::Vector3f> const boundary_entity_locations{boundary_entity_centre};
    FTraceFixture const boundary_fixture{boundary_entity_locations,
                                         aabb_half_extents,
                                         ml::simulation::Vector3f{},
                                         fixture_grid_dims,
                                         fixture_cell_dims};
    std::vector<FExpectedTrace> const boundary_cases{
    ...
    ml::simulation::collision::CellCoord const single_cell_grid_dims{1, 1, 1};
    ml::simulation::Vector3f const single_cell_dims{{100.f, 120.f, 140.f}};
    ml::simulation::Vector3f const single_cell_half_extents{{10.f, 10.f, 10.f}};
    std::vector<ml::simulation::Vector3f> const single_cell_locations{ml::simulation::Vector3f{}};
    ...
    std::vector<FExpectedTrace> const single_cell_cases{
...
void FCollisionUniformGridTraceScenario::test_rebuild_lifecycle() {
    ml::simulation::Vector3f const initial_location{{-150.f, 0.f, 0.f}};
    ml::simulation::Vector3f const moved_location{{150.f, 0.f, 0.f}};
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    constexpr float trace_offset{25.f};

    std::vector<ml::simulation::Vector3f> const initial_locations{initial_location};
    ...
    std::vector<FExpectedTrace> const initial_cases{
    ...
    std::vector<ml::simulation::Vector3f> const moved_locations{moved_location};
    std::vector<std::uint8_t> const alive{std::uint8_t{1}};
    fixture.update_entities(moved_locations, alive);
    std::vector<FExpectedTrace> const moved_cases{
    ...
    std::vector<FExpectedTrace> const dead_cases{
    ...
    ml::simulation::Vector3f const replacement_location{{250.f, 0.f, 0.f}};
    ...
    ml::simulation_tests::expect_true(old_handle.generation != replacement_handle.generation,
                                      "Replacement entity advances registry generation");
    ml::simulation_tests::expect_true(fixture.registry.is_stale(old_handle),
                                      "Reused collision handle becomes stale");

    std::vector<ml::simulation::Vector3f> const replacement_starts{
        {{replacement_location.X - trace_offset, 0.f, 0.f}},
    };
    std::vector<ml::simulation::Vector3f> const replacement_ends{
        {{replacement_location.X + trace_offset, 0.f, 0.f}},
    };
    ...
    if (replacement_hits.hits[0] != 0) {
        ml::simulation_tests::expect_equal(replacement_handle,
                                           replacement_hits.entities[0],
                                           "Trace resolves replacement generation");
    }

    std::vector<ml::simulation::Vector3f> const sparse_locations{
        {{-250.f, 0.f, 0.f}},
        {{0.f, 0.f, 0.f}},
        {{250.f, 0.f, 0.f}},
    };
    ...
    std::vector<FExpectedTrace> const sparse_cases{
    ...
    ml::simulation::Vector3f const sparse_replacement_location{{0.f, 200.f, 0.f}};
    ...
    ml::simulation_tests::expect_equal(sparse_old_handle.index,
                                       sparse_replacement_handle.index,
                                       "Sparse replacement reuses middle registry slot");
    ml::simulation_tests::expect_true(sparse_old_handle.generation !=
                                          sparse_replacement_handle.generation,
                                      "Sparse replacement advances middle registry generation");

    std::vector<ml::simulation::Vector3f> const sparse_replacement_starts{
        sparse_replacement_location - ml::simulation::Vector3f{{0.f, trace_offset, 0.f}},
    };
    std::vector<ml::simulation::Vector3f> const sparse_replacement_ends{
        sparse_replacement_location + ml::simulation::Vector3f{{0.f, trace_offset, 0.f}},
    };
    ...
    if (sparse_replacement_hits.hits[0] != 0) {
        ml::simulation_tests::expect_equal(sparse_replacement_handle,
...
void FCollisionUniformGridTraceScenario::test_deterministic_reference_sweep() {
    ...
    ml::simulation::Vector3f const targeted_trace_extent{{1000.f, 1000.f, 1000.f}};
    ml::simulation::Vector3f const local_aabb_centre{{3.f, -5.f, 7.f}};
    ml::simulation::Vector3f const aabb_half_extents{{7.f, 11.f, 13.f}};
    ...
    auto const run_sweep{[local_aabb_centre, aabb_half_extents, targeted_trace_extent](
        ...
        std::vector<ml::simulation::Vector3f> entity_locations{};
        ...
        for (std::int32_t i_trace{}; i_trace < trace_count; ++i_trace) {
            ...
            ml::simulation_tests::expect_equal(expected_hit,
                                               hits.hits[i_trace],
            ...
            ml::simulation_tests::expect_equal(fixture.handles[nearest_entity],
        ...
        ml::simulation_tests::expect_true(expected_hit_count >= entity_count,
                                          "Reference sweep contains targeted hits");
        ml::simulation_tests::expect_true(expected_hit_count < trace_count,
...
void FCollisionUniformGridTraceScenario::test_invariance_properties() {
    ml::simulation::Vector3f const aabb_half_extents{{18.f, 22.f, 15.f}};
    ml::simulation::Vector3f const local_aabb_centre{{4.f, -3.f, 5.f}};
    std::vector<ml::simulation::Vector3f> const entity_locations{
        {{-220.f, -40.f, 10.f}},
        {{35.f, 70.f, -20.f}},
        {{210.f, -120.f, 80.f}},
    ...
    auto const compare_same_entity_order{[&baseline_hits](FTraceHits const& candidate_hits,
        ...
        for (std::int32_t i{}; i < count; ++i) {
            ...
            ml::simulation_tests::expect_distance_near(baseline_hits.locations[i],
    ...
    std::vector<ml::simulation::Vector3f> permuted_starts{};
    std::vector<ml::simulation::Vector3f> permuted_ends{};
    ...
    for (std::int32_t i{}; i < trace_count; ++i) {
        ...
        ml::simulation_tests::expect_equal(baseline_hits.hits[source_index],
        ...
        ml::simulation_tests::expect_equal(baseline_hits.entities[source_index],
                                           permuted_hits.entities[i],
                                           "Trace permutation preserves entity",
                                           i);
        ml::simulation_tests::expect_distance_near(baseline_hits.locations[source_index],
    ...
    std::vector<ml::simulation::Vector3f> const reversed_entity_locations{
        entity_locations[2],
        entity_locations[1],
        entity_locations[0],
    };
    ...
    for (std::int32_t i{}; i < trace_count; ++i) {
        ml::simulation_tests::expect_equal(baseline_hits.hits[i],
    ...
    ml::simulation::Vector3f const translation{{100.f, 0.f, 0.f}};
    std::vector<ml::simulation::Vector3f> translated_entity_locations{};
    std::vector<ml::simulation::Vector3f> translated_starts{};
    std::vector<ml::simulation::Vector3f> translated_ends{};
    ...
    for (std::int32_t i{}; i < trace_count; ++i) {
        ...
        if (baseline_hits.hits[i] != 0 && translated_hits.hits[i] != 0) {
            ml::simulation_tests::expect_equal(baseline_hits.entities[i].index,
                                               translated_hits.entities[i].index,
                                               "Whole-cell translation preserves entity",
                                               i);
            ml::simulation_tests::expect_distance_near(baseline_hits.locations[i] + translation,
...
void FCollisionUniformGridTraceScenario::test_empty_batches_and_output_reuse() {
    ml::simulation::Vector3f const aabb_half_extents{{10.f, 10.f, 10.f}};
    std::vector<ml::simulation::Vector3f> const no_entities;
    FTraceFixture const empty_fixture{no_entities, aabb_half_extents};
    std::vector<ml::simulation::Vector3f> const no_traces;
    auto const no_results{run_traces(empty_fixture, no_traces, no_traces)};
    ml::simulation_tests::expect_equal(0, no_results.num(), "Empty grid accepts empty trace batch");

    std::vector<ml::simulation::Vector3f> const miss_starts{{{-20.f, 0.f, 0.f}},
                                                            {{0.f, -20.f, 0.f}}};
    std::vector<ml::simulation::Vector3f> const miss_ends{{{20.f, 0.f, 0.f}}, {{0.f, 20.f, 0.f}}};
    ...
    std::vector<ml::simulation::Vector3f> const entity_locations{ml::simulation::Vector3f{}};
    ...
    ml::simulation::LineTraces hit_trace;
    ...
    ml::simulation::LineTraces miss_trace;
    ...
    ml::simulation_tests::expect_equal(populated_fixture.handles[0],
...
void FCollisionUniformGridTraceScenario::test_dense_and_wide_aabbs() {
    ...
    ml::simulation::Vector3f const dense_location{{25.f, 25.f, 25.f}};
    ml::simulation::Vector3f const dense_half_extents{{1.f, 1.f, 1.f}};
    std::vector<ml::simulation::Vector3f> dense_locations{};
    ...
    std::vector<FExpectedTrace> const dense_cases{
    ...
    ml::simulation::Vector3f const wide_half_extents{{160.f, 160.f, 160.f}};
    std::vector<ml::simulation::Vector3f> const wide_locations{ml::simulation::Vector3f{}};
    ...
    std::vector<FExpectedTrace> const wide_cases{
...
void FCollisionUniformGridTraceScenario::test_production_scale() {
    ...
    ml::simulation::Vector3f const aabb_half_extents{{1000.f, 1000.f, 1000.f}};
    ml::simulation::Vector3f const negative_entity{{-999000.f, 0.f, 0.f}};
    ml::simulation::Vector3f const positive_entity{{999000.f, 0.f, 0.f}};
    std::vector<ml::simulation::Vector3f> const entity_locations{negative_entity, positive_entity};
    FTraceFixture const fixture{
        entity_locations, aabb_half_extents, ml::simulation::Vector3f{}, grid_dims, cell_dims};
    std::vector<FExpectedTrace> const cases{
...
void FCollisionUniformGridTraceScenario::test_static_geometry() {
    ml::simulation::Vector3f const dynamic_location{{100.f, 0.f, 0.f}};
    ml::simulation::Vector3f const half_extents{{10.f, 10.f, 10.f}};
    std::vector<ml::simulation::Vector3f> const dynamic_locations{dynamic_location};
    FTraceFixture fixture{dynamic_locations, half_extents};

    auto set_static_aabb{[&fixture](ml::simulation::Vector3f const min_point,
                                    ml::simulation::Vector3f const max_point) {
        simulation::collision::WorldAABBs static_aabbs;
        simulation::collision::add(static_aabbs, min_point, max_point);
        fixture.grid.set_static_aabbs(std::move(static_aabbs));
    }};
    std::vector<ml::simulation::Vector3f> const starts{{{-200.f, 0.f, 0.f}}};
    std::vector<ml::simulation::Vector3f> const ends{{{200.f, 0.f, 0.f}}};

    set_static_aabb({{-60.f, -10.f, -10.f}}, {{-40.f, 10.f, 10.f}});
    auto static_hits{run_traces(fixture, starts, ends)};
    ml::simulation_tests::expect_equal(
        std::uint8_t{1}, static_hits.hits[0], "Static AABB is traceable");
    ml::simulation_tests::expect_true(!static_hits.entities[0].is_valid(),
    ...
    ml::simulation_tests::expect_distance_near(ml::simulation::Vector3f{{-60.f, 0.f, 0.f}},
                                               static_hits.locations[0],
                                               hit_location_tolerance,
                                               "Static hit reports nearest entry point");

    set_static_aabb({{-60.f, 100.f, -10.f}}, {{-40.f, 120.f, 10.f}});
    std::vector<ml::simulation::Vector3f> const offset_starts{{{-200.f, 80.f, 0.f}}};
    std::vector<ml::simulation::Vector3f> const offset_ends{{{200.f, 80.f, 0.f}}};
    ...
    auto const sweep_hits{run_sweeps(
        fixture, offset_starts, offset_ends, ml::simulation::Vector3f{{20.f, 20.f, 20.f}})};
    ml::simulation_tests::expect_equal(
        std::uint8_t{1}, sweep_hits.hits[0], "AABB sweep detects static geometry");
    ml::simulation_tests::expect_distance_near(ml::simulation::Vector3f{{-80.f, 80.f, 0.f}},
                                               sweep_hits.locations[0],
                                               hit_location_tolerance,
                                               "AABB sweep reports expanded entry point");

    std::vector<ml::simulation::Vector3f> const fighter_locations{{{-100.f, 0.f, 0.f}}};
    std::vector<ml::simulation::EntityType> const fighter_types{
        ml::simulation::EntityType::CapitalShipFighter};
    ...
    fighter_fixture.set_entity_aabb(
        ml::simulation::EntityType::CapitalShipFighter, ml::simulation::Vector3f{}, half_extents);
    ...
    auto const fighter_masked_hits{
        run_sweeps(fighter_fixture, starts, ends, ml::simulation::Vector3f{{20.f, 20.f, 20.f}})};
    ml::simulation_tests::expect_equal(fighter_fixture.handles[0],
    ...
    auto const static_geometry_hits{
        run_sweeps(fighter_fixture,
                   starts,
                   ends,
                   ml::simulation::Vector3f{{20.f, 20.f, 20.f}},
                   {},
                   ioj::ETraceEntityFilter::ExcludeCapitalShipFighters)};
    ml::simulation_tests::expect_equal(std::uint8_t{1},
                                       static_geometry_hits.hits[0],
                                       "Static geometry remains after fighter exclusion");
    ml::simulation_tests::expect_true(!static_geometry_hits.entities[0].is_valid(),
    ...
    ml::simulation_tests::expect_equal(fixture.handles[0],
                                       dynamic_hits.entities[0],
                                       "Closer dynamic geometry wins over static geometry");
    ml::simulation_tests::expect_equal(std::int32_t{-1},
                                       dynamic_hits.static_geometry_indices[0],
                                       "Dynamic hit clears static identity");

    std::vector<FRegistryEntityHandle> const ignored_entities{fixture.handles[0]};
    auto const ignored_dynamic_hits{run_traces(fixture, starts, ends, ignored_entities)};
    ml::simulation_tests::expect_true(!ignored_dynamic_hits.entities[0].is_valid(),
    ...
    ml::simulation_tests::expect_equal(runtime_static_index,
...
void run_collision_uniform_grid_trace(ml::simulation_tests::SimulationFixture const&,
                                      ECollisionUniformGridTraceScenario const scenario) {
    FCollisionUniformGridTraceScenario runner{scenario};
    runner.run();
}
}
