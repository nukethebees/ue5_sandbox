...
void run_worldless_fighter_obstacle_avoidance(ioj::sim::tests::SimulationFixture const& config) {
    ...
    ioj::sim::Vector3f const obstacle_min{{-1000.f, -1500.f, -1500.f}};
    ioj::sim::Vector3f const obstacle_max{{1000.f, 1500.f, 1500.f}};
    ...
    ioj::sim::collision::set(data.static_bounds, 0, obstacle_min, obstacle_max);
    ioj::sim::tests::add_capital_spawn(data,
                                       ioj::sim::Vector3f{{source_x, -7000.f, 0.f}},
                                       ioj::sim::Team::Green,
                                       1,
                                       0.f,
                                       60.f,
    ...
    ioj::sim::tests::expect_true(harness.run_until_timeline_finished(15.0),
                                 "Fighter obstacle-avoidance timeline completes");

    ioj::sim::tests::expect_true(fighter_spawned, "Avoidance fighter spawned");
    ioj::sim::tests::expect_true(!entered_expanded_obstacle,
                                 "Fighter remains outside obstacle clearance bounds");
    ioj::sim::tests::expect_true(maximum_lateral_distance > expanded_max.Y,
                                 "Fighter steers visibly around the obstacle");
    ioj::sim::tests::expect_true(maximum_x > expanded_max.X,
                                 "Fighter progresses past the obstacle");
}

auto make_fighter_navigation_test_data(ioj::sim::tests::SimulationFixture const& config,
                                       std::vector<ioj::sim::Transform3d> spawn_slots,
                                       ioj::sim::Vector3f const source_location,
                                       ioj::sim::Vector3f const target_location)
    -> LevelSimInitData {
    auto data{ioj::sim::tests::make_simulation_data(config)};
    data.fighters.health = fighter_navigation_test::collision_resilient_health;
    data.fighters.speed = 4000.f;
    ...
    ioj::sim::tests::add_capital_spawn(
        data, source_location, ioj::sim::Team::Green, 1, 0.f, 60.f, 100000);
    ioj::sim::tests::add_capital_spawn(
        data, target_location, ioj::sim::Team::Red, -1, 60.f, 60.f, 100000);
    return data;
}

void run_worldless_fighter_capital_obstruction(ioj::sim::tests::SimulationFixture const& config) {
    ioj::sim::Vector3f const source{{-18000.f, -7000.f, 0.f}};
    ioj::sim::Vector3f const obstacle{{0.f, 0.f, 0.f}};
    ioj::sim::Vector3f const target{{18000.f, 0.f, 0.f}};
    auto data{ioj::sim::tests::make_simulation_data(config)};
    data.fighters.health = fighter_navigation_test::collision_resilient_health;
    data.fighters.speed = 4000.f;
    data.fighters.avoidance_lookahead_time = 1.f;
    ...
    data.capital_ships.fighter_spawn_slots_relative_transforms = {
        {.location = {3000.0, 7000.0, 0.0}}};
    ioj::sim::tests::add_capital_spawn(data, source, ioj::sim::Team::Green, 2, 0.f, 60.f, 100000);
    ioj::sim::tests::add_capital_spawn(
    ...
    auto const obstacle_bounds{ioj::sim::collision::make_entity_world_bounds(
        data.entity_bounds,
        ioj::sim::collision::EntityAABBs::capital_ship_index,
        obstacle,
        ioj::sim::to_quaternion(obstacle_rotation))};
    ...
    ioj::sim::Vector3f const clearance_extent{{clearance, clearance, clearance}};
    ioj::sim::tests::WorldlessSimulationTest harness{std::move(data)};
    harness.finish_initialisation();
    auto const& fighters{harness.get_simulation().get_fighters()};
    ...
    ioj::sim::tests::expect_true(!entered_obstacle,
                                 "Fighter remains outside capital clearance bounds");
    ioj::sim::tests::expect_true(saw_avoidance,
                                 "Fighter selects hard avoidance around the capital");
    ioj::sim::tests::expect_true(maximum_lateral_distance > capital_half_extent.Y,
...
void run_worldless_fighter_clear_navigation(ioj::sim::tests::SimulationFixture const& config) {
    auto data{make_fighter_navigation_test_data(
        config,
        {ioj::sim::Transform3d{.location = ml::Vector3d{3000.f, 7000.f, 0.f}}},
        ioj::sim::Vector3f{{-20000.f, -7000.f, 0.f}},
        ioj::sim::Vector3f{{20000.f, 0.f, 0.f}})};
    ioj::sim::tests::WorldlessSimulationTest harness{std::move(data)};
    harness.finish_initialisation();
    auto const& fighters{harness.get_simulation().get_fighters()};
    ioj::sim::Vector3f first_location{ioj::sim::Vector3f{}};
    ioj::sim::Vector3f last_location{ioj::sim::Vector3f{}};
    bool recorded_first{};
    ...
    ioj::sim::tests::expect_true(recorded_first, "Clear-path fighter spawned");
    ioj::sim::tests::expect_true(scanned_before_first_movement,
                                 "New fighter scans obstacles before its first movement");
    ioj::sim::tests::expect_true(last_location.X > first_location.X + 1000.f,
                                 "Clear-path fighter advances directly");
    ioj::sim::tests::expect_true(std::abs(last_location.Y - first_location.Y) < 1.f &&
                                     std::abs(last_location.Z - first_location.Z) < 1.f,
                                 "Clear path has no lateral steering");
    ioj::sim::tests::expect_true(!saw_separation, "Clear path never applies separation");
    ioj::sim::tests::expect_true(!saw_avoidance, "Clear path never applies hard avoidance");
}

void run_worldless_fighter_separation(ioj::sim::tests::SimulationFixture const& config) {
    auto data{make_fighter_navigation_test_data(
        config,
        {ioj::sim::Transform3d{.location = ml::Vector3d{3000.f, 6950.f, 0.f}},
         ioj::sim::Transform3d{.location = ml::Vector3d{3000.f, 7050.f, 0.f}}},
        ioj::sim::Vector3f{{-20000.f, -7000.f, 0.f}},
        ioj::sim::Vector3f{{20000.f, 0.f, 0.f}})};
    ioj::sim::tests::WorldlessSimulationTest harness{std::move(data)};
    harness.finish_initialisation();
    ...
    harness.on_end_tick = [&](LevelSim&) {
        ...
        final_distance = ioj::sim::tests::distance(locations[0], locations[1]);
    ...
    ioj::sim::tests::expect_true(saw_separation, "Both close fighters apply separation");
    ioj::sim::tests::expect_true(final_distance > initial_distance + 500.f,
...
auto measure_fighter_cluster(std::span<ioj::sim::Vector3f const> const locations,
    ...
    ioj::sim::Vector3f centroid{ioj::sim::Vector3f{}};
    ...
    for (std::int32_t i{}; i < static_cast<std::int32_t>(locations.size()); ++i) {
        ...
        centroid_distances.push_back(ioj::sim::tests::distance(locations[i], centroid));
        for (std::int32_t j{}; j < static_cast<std::int32_t>(locations.size()); ++j) {
            if (i == j) {
                continue;
            }
            auto const distance{ioj::sim::tests::distance(locations[i], locations[j])};
...
auto run_dense_navigation_fixture(ioj::sim::tests::SimulationFixture const& config,
    ...
    result.collision_distance =
        ioj::sim::collision::get_entity_radius(data.entity_bounds,
                                               ioj::sim::collision::EntityAABBs::fighter_index) *
        2.f;
    ioj::sim::tests::WorldlessSimulationTest harness{std::move(data)};
    harness.finish_initialisation();
    auto const& fighters{harness.get_simulation().get_fighters()};
...
void run_worldless_fighter_dense_determinism(ioj::sim::tests::SimulationFixture const& config) {
    ...
    ioj::sim::tests::expect_true(first_result.timeline_completed,
                                 "First dense fixture timeline completes");
    ioj::sim::tests::expect_true(second_result.timeline_completed,
    ...
    ioj::sim::tests::expect_equal(static_cast<std::int32_t>(first.size()),
                                  static_cast<std::int32_t>(second.size()),
                                  "Deterministic fixtures have equal counts");
    ioj::sim::tests::expect_equal(first_result.collision_distance,
    ...
    for (std::int32_t i{}; i < static_cast<std::int32_t>(first.size()); ++i) {
        ioj::sim::tests::expect_true(!(!std::isfinite(first[i].X) || !std::isfinite(first[i].Y) ||
                                       !std::isfinite(first[i].Z)),
                                     "Dense fighter location is finite",
                                     i);
        if ((i >= 0 && static_cast<std::size_t>(i) < second.size())) {
            ioj::sim::tests::expect_true((std::abs(first[i].X - second[i].X) <= 0.f &&
    ...
    ioj::sim::tests::expect_true(metrics.finite, "Every dense fighter remains finite");
    ioj::sim::tests::expect_less_equal(
    ...
    ioj::sim::tests::expect_less_equal(first_result.collision_distance * 0.25f,
                                       metrics.median_nearest_neighbour_distance,
                                       "Dense fighters establish meaningful local spacing");
    ioj::sim::tests::expect_true(first_result.saw_immediate_risk &&
                                     second_result.saw_immediate_risk,
                                 "Coincident groups enter the immediate-risk tier");
    ioj::sim::tests::expect_equal(first_result.query_count,
...
void run_worldless_fighter_large_cluster(ioj::sim::tests::SimulationFixture const& config) {
    ...
    ioj::sim::tests::expect_true(result.timeline_completed, "Large-cluster timeline completes");
    ioj::sim::tests::expect_equal(fighter_count,
                                  static_cast<std::int32_t>(result.locations.size()),
                                  "Large cluster retains every fighter");
    ioj::sim::tests::expect_true(metrics.finite, "Large-cluster positions remain finite");
    ioj::sim::tests::expect_less_equal(
    ...
    ioj::sim::tests::expect_less_equal(result.collision_distance * 0.25f,
                                       metrics.median_nearest_neighbour_distance,
                                       "Large cluster establishes meaningful local spacing");
    ioj::sim::tests::expect_true(result.query_count > 0 && result.saw_immediate_risk,
...
void run_worldless_fighter_hard_avoidance_authority(
    ...
    ioj::sim::Vector3f const obstacle_min{{-1000.f, -1000.f, -1500.f}};
    ioj::sim::Vector3f const obstacle_max{{1000.f, 1000.f, 1500.f}};
    auto data{make_fighter_navigation_test_data(
        config,
        {ioj::sim::Transform3d{.location = ml::Vector3d{3000.f, 31500.f, 0.f}},
         ioj::sim::Transform3d{.location = ml::Vector3d{3000.f, 31600.f, 0.f}}},
        ioj::sim::Vector3f{{-15000.f, -30000.f, 0.f}},
        ioj::sim::Vector3f{{15000.f, 0.f, 0.f}})};
    ...
    ioj::sim::tests::expect_true(harness.run_until_timeline_finished(11.0),
                                 "Hard-authority timeline completes");
    ioj::sim::tests::expect_true(saw_separation, "Obstacle fixture applies soft separation");
    ioj::sim::tests::expect_true(saw_avoidance, "Hard avoidance overrides unsafe soft steering");
    ioj::sim::tests::expect_true(!entered_obstacle,
...
void run_worldless_fighter_navigation_frequency(ioj::sim::tests::SimulationFixture const& config) {
    ...
    ioj::sim::tests::expect_true(saw_clear, "Clear fighter demotes to the clear-risk tier");
    ioj::sim::tests::expect_true(dense_result.saw_immediate_risk,
                                 "Clustered fighters promote to immediate risk");
    ioj::sim::tests::expect_true(dense_result.query_count > clear_queries * 4,
...
void run_worldless_fighter_attack(ioj::sim::tests::SimulationFixture const& config) {
    ...
    ioj::sim::tests::add_capital_spawn(
        data, ioj::sim::Vector3f{{-22020.f, 2170.f, 4360.f}}, ioj::sim::Team::Green, 1, 0.f, 60.f);
    ioj::sim::tests::add_capital_spawn(
        data, ioj::sim::Vector3f{{17030.f, 2170.f, 4360.f}}, ioj::sim::Team::Red, 0, 60.f, 60.f);
    ...
    struct Sample {
        ...
        std::vector<ioj::sim::Team> fighter_teams{};
    };
    ...
    ioj::sim::tests::expect_true(harness.run_until_timeline_finished(12.0),
                                 "Fighter-attack timeline completes");
    ioj::sim::tests::expect_true(!samples.is_empty(), "Fighter-attack samples are recorded");
    if (samples.is_empty()) {
        return;
    }
    ...
    ioj::sim::tests::expect_greater(static_cast<std::int32_t>(before.fighter_teams.size()),
                                    std::int32_t{0},
                                    "Hero fighters spawned");
    for (std::int32_t i{}; i < static_cast<std::int32_t>(before.fighter_teams.size()); ++i) {
        ioj::sim::tests::expect_equal(
            ioj::sim::Team::Green, before.fighter_teams[i], "Fighter is on the hero team", i);
    }
    ioj::sim::tests::expect_true(after.enemy_health < before.enemy_health, "Enemy lost health");
}
