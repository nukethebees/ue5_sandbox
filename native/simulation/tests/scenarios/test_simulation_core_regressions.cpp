...
void run_worldless_simulation_core_regression(ml::simulation_tests::SimulationFixture const& config,
    ...
    if (scenario == ESimulationCoreRegressionScenario::DamageLifecycle) {
        ...
        data.capital_spawns.teams[0] =
            static_cast<ml::simulation::Team>(ml::simulation::Team::White);
    ...
    if (scenario == ESimulationCoreRegressionScenario::FixedTickLifecycle) {
        ...
        ml::simulation_tests::expect_equal(std::uint64_t{0},
                                           simulation.get_clock().get_completed_ticks(),
                                           "Half tick is accumulated");
        simulation.advance(period * 0.5);
        ml::simulation_tests::expect_equal(std::uint64_t{1},
                                           simulation.get_clock().get_completed_ticks(),
        ...
        ml::simulation_tests::expect_equal(std::uint64_t{4},
                                           simulation.get_clock().get_completed_ticks(),
                                           "Large delta catches up deterministically");
        ml::simulation_tests::expect_equal(
            4, end_tick_calls, "Catch-up executes every end-tick hook");
        ml::simulation_tests::expect_equal(period * 4.0,
        ...
        ml::simulation_tests::expect_equal(std::uint64_t{5},
                                           simulation.get_clock().get_completed_ticks(),
                                           "Resume preserves accumulated fraction");
        ml::simulation_tests::expect_equal(5, end_tick_calls, "Resumed tick executes one hook");
        return;
    }
    ...
    ml::simulation_tests::expect_true(!samples.is_empty(), "Damage lifecycle samples are recorded");
    if (samples.is_empty()) {
        return;
    }
    ...
    ml::simulation_tests::expect_equal(1, initial.capital_count, "One capital starts active");
    ml::simulation_tests::expect_equal(
    ...
    ml::simulation_tests::expect_equal(75, nonlethal.health, "Nonlethal damage is applied once");
    ml::simulation_tests::expect_equal(
    ...
    ml::simulation_tests::expect_true(harness.get_registry().is_valid_dead(damaged_handle),
...
void run_worldless_collision_damage(ml::simulation_tests::SimulationFixture const& config) {
    ...
    data.capital_ships.fighter_spawn_slots = 0;
    data.capital_ships.fighter_spawn_slots_relative_transforms.clear();
    data.player = ml::simulation_tests::make_player_spawn(config);
    ...
    if (!ml::simulation_tests::expect_not_null(player,
    ...
    ml::simulation_tests::expect_greater(samples.num(), 2, "Three collision ticks are recorded");
    ...
    ml::simulation_tests::expect_equal(
        1, first.dynamic_overlap_count, "First tick detects one overlap");
    ml::simulation_tests::expect_equal(
        1, second.dynamic_overlap_count, "Second tick detects one overlap");
    ml::simulation_tests::expect_equal(
        1, third.dynamic_overlap_count, "Third tick detects one overlap");
    ml::simulation_tests::expect_equal(collision_damage_test::player_health -
    ...
    ml::simulation_tests::expect_equal(collision_damage_test::capital_health -
                                           collision_damage_test::overlap_damage,
    ...
    ml::simulation_tests::expect_equal(0, third.capital_health, "Third overlap kills the capital");
    ml::simulation_tests::expect_true(third.player_alive, "Player survives the third overlap tick");
    ml::simulation_tests::expect_true(harness.get_registry().is_valid_dead(capital_handle),
