...
TEST(NativeSimulation, GameMemoryTest) {
    ...
    ioj::sim::tests::expect_true(memory.owns_backing_locally(),
    ...
    std::byte* first_address{};
    {
        ...
        ioj::sim::tests::expect_equal(memory.get_stats().live_block_bytes,
                                      std::size_t{128},
                                      "A live block contributes its requested bytes");
    }
    auto reused{memory.acquire_block(128, 64)};
    ioj::sim::tests::expect_equal(reused.data(), first_address, "A returned exact block is reused");

    auto exhaustion{memory.try_acquire_block(1024, 32)};
    ioj::sim::tests::expect_false(exhaustion.has_value(),
                                  "Root exhaustion is nonfatal through the try API");
    auto const failure{memory.get_last_allocation_failure()};
    ioj::sim::tests::expect_true(failure.has_value(), "Root exhaustion records a diagnostic");
    ...
    ioj::sim::tests::expect_true(first_lease.has_value(), "The first backing lease succeeds");
    ioj::sim::tests::expect_false(backing->try_acquire_lease().has_value(),
    ...
    ioj::sim::tests::expect_false(static_cast<bool>(first_lease.value()),
                                  "A moved-from lease is empty");
    first_lease.reset();
    ioj::sim::tests::expect_true(backing->is_leased(),
                                 "The moved lease keeps the backing occupied");
    moved_lease = {};

    auto second_lease{backing->try_acquire_lease()};
    ioj::sim::tests::expect_true(second_lease.has_value(), "A sequential backing lease succeeds");
    ioj::sim::tests::expect_equal(second_lease->get().data(),
                                  backing_address,
    ...
    second_lease.reset();
    {
        ...
        ioj::sim::tests::expect_true(external_lease.has_value(),
        ...
        ioj::sim::tests::expect_false(external_memory.owns_backing_locally(),
                                      "External game memory does not own its backing locally");
        ioj::sim::tests::expect_equal(external_memory.backing_address(),
                                      backing_address,
                                      "External game memory uses the leased backing");
    }
    ioj::sim::tests::expect_false(backing->is_leased(),
