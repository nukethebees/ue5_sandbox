...
struct RegistryEntityHandle {
    ...
    RegistryEntityHandle() = default;
    constexpr RegistryEntityHandle(index_type const index, generation_type const generation)
        : index(index)
        , generation(generation) {}

    auto operator<=>(RegistryEntityHandle const&) const noexcept = default;
