...
struct SingleAllocationRegistryEntityDataStorage
    ...
    using SchemaConstView = RegistryEntityDataConstView;
    using ml::native_soa::StorageOperations::append_from;
    auto append_from(RegistryEntityDataConstView const& source) -> size_type {
        source.validate_array_sizes();
        auto const count{source.num()};
  ...
  private:
    ...
    auto ordinary_source_aliases_storage(RegistryEntityDataConstView const& source) const noexcept
        -> bool {
        if (data_ == nullptr) {
            return false;
        }
        ...
        return aliases(source.locations.xs) || aliases(source.locations.ys) ||
               aliases(source.locations.zs) || aliases(source.velocities.xs) ||
               ...
               aliases(source.rotations.rolls.data()) || aliases(source.healths.data()) ||
               aliases(source.teams.data()) || aliases(source.entity_types.data());
    }
