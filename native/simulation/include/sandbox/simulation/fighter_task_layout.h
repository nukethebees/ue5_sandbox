...
inline constexpr auto task_type_count{static_cast<std::size_t>(CapitalShipFighterTask::COUNT)};
using TaskCounts = std::array<std::int32_t, task_type_count>;
using TaskSpans = std::array<FIndexSpan, task_type_count>;
...
class TaskLayout {
  public:
    ...
    [[nodiscard]] auto next_index(CapitalShipFighterTask const task) noexcept -> std::int32_t {
...
[[nodiscard]] auto count_tasks(std::span<CapitalShipFighterTask const> tasks) -> TaskCounts;
[[nodiscard]] auto tasks_are_contiguous(std::span<CapitalShipFighterTask const> tasks,
