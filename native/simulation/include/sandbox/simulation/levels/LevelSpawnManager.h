...
class FLevelSpawnManager {
  public:
    ...
    void set_entity_handle(std::int32_t entity_index, FRegistryEntityHandle handle);
    ...
    auto get_handle(std::int32_t entity_index) const -> FRegistryEntityHandle;
    auto get_entity_handles() const noexcept -> std::span<FRegistryEntityHandle const> {
  ...
  private:
    ...
    std::vector<FRegistryEntityHandle> entity_handles_{};
    std::vector<FRegistryEntityHandle> target_handles_scratch_{};
};
}
