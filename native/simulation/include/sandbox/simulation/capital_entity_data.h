...
struct CapitalEntityDataConstView {
    ...
    std::span<FIndexSpan const> capital_fighter_handle_spans;
    ...
    void each_column(Fn&& fn) const {
        ...
        fn(teams);
        fn(healths);
        fn(capital_fighter_handle_spans);
        fn(target_handles);
    }
    ...
    auto slice(size_type const offset, size_type const count) const -> CapitalEntityDataConstView {
        ...
        return {
            ...
            capital_fighter_handle_spans.subspan(static_cast<std::size_t>(offset),
                                                 static_cast<std::size_t>(count)),
...
struct CapitalEntityDataView {
    ...
    std::span<FIndexSpan> capital_fighter_handle_spans;
...
struct CapitalEntityData {
    ...
    ml::native_soa::Vector<FIndexSpan> capital_fighter_handle_spans;
    ...
    void reserve(size_type const count) {
        ...
        capital_fighter_handle_spans.reserve(static_cast<std::size_t>(count));
    ...
    void reset() noexcept {
        ...
        teams.clear();
        healths.clear();
        capital_fighter_handle_spans.clear();
    ...
    void set_num(size_type const count) {
        ...
        teams.resize(size);
        healths.resize(size);
        capital_fighter_handle_spans.resize(size);
    ...
    void remove_at_swap(size_type const index, size_type const count) {
        ...
        for (size_type i{}; i < moved; ++i) {
            capital_fighter_handle_spans[index + i] = capital_fighter_handle_spans[source + i];
        }
        for (size_type i{}; i < moved; ++i) {
    ...
    void append_from(ConstView source) {
        ...
        {
            auto const address{
                reinterpret_cast<std::uintptr_t>(source.capital_fighter_handle_spans.data())};
            auto const begin{reinterpret_cast<std::uintptr_t>(capital_fighter_handle_spans.data())};
            ml::native_soa::require(address < begin ||
                                    address >= begin + capital_fighter_handle_spans.size() *
                                                           sizeof(FIndexSpan));
        }
        {
        ...
        capital_fighter_handle_spans.insert(capital_fighter_handle_spans.end(),
                                            source.capital_fighter_handle_spans.begin(),
                                            source.capital_fighter_handle_spans.end());
    ...
    void copy_element(size_type const dst_index, Other const& other, size_type const src_index) {
        ...
        capital_fighter_handle_spans[static_cast<std::size_t>(dst_index)] =
            other.capital_fighter_handle_spans[static_cast<std::size_t>(src_index)];
