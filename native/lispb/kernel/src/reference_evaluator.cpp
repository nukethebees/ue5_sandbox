...
auto fixture_value(std::size_t const operand_index, std::size_t const element_index) -> T {
    if constexpr (std::is_same_v<T, std::uint32_t>) {
        ...
        constexpr std::array<long double, 12> values{1.0L,
                                                     -2.0L,
                                                     0.25L,
                                                     -0.5L,
                                                     3.0L,
        ...
        if constexpr (std::is_signed_v<T>) {
            if (lhs == std::numeric_limits<T>::min() && rhs == T{-1}) {
                throw std::invalid_argument{
                    "generated reference fixture overflows signed division"};
            }
        }
...
auto evaluate(Expression const& expression,
    ...
    if (expression.kind == ExpressionKind::reference) {
        auto const found{
            std::ranges::find_if(expanded.operation->operands, [&](auto const& operand) {
        ...
        auto const value_index{
            expanded.storage[operand_index] == StorageKind::array ? element_index : std::size_t{0}};
...
auto make_typed_fixture(ExpandedVariant const& expanded, std::size_t const count)
    ...
    for (std::size_t operand_index{}; operand_index < expanded.operation->operands.size();
        ...
        auto const value_count{
            expanded.storage[operand_index] == StorageKind::array ? count : std::size_t{1}};
