...
auto selected_variant(std::vector<ExpandedVariant> const& variants, Emission const& emission)
    -> ExpandedVariant const& {
...
auto render(KernelModule const& module, Profile const profile)
    ...
    auto const emission{std::ranges::find_if(
        module.emissions, [&](auto const& candidate) { return candidate.profile == profile; })};
    ...
    if (profile == Profile::unreal_avx2_lab) {
        ...
        return {
            codegen::GeneratedFile{emission->header, render_avx2_lab_header(*emission, selected)},
            codegen::GeneratedFile{emission->source, render_avx2_lab_source(*emission, selected)}};
    }
    ...
    if (profile == Profile::standard) {
        std::vector<codegen::GeneratedFile> result{
            ...
            codegen::GeneratedFile{emission->source,
                                   render_standard_source(module, *emission, variants)}};
        if (emission->tests) {
            result.push_back(codegen::GeneratedFile{
