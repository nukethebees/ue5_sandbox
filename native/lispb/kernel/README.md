...
```text
out/build/<preset>/native/lispb/kernel/native-simd-generated/native/generated/
...
`native/lispb/kernel/benchmarks/add_scaled_benchmarks.cpp` and
`native/lispb/kernel/benchmarks/dot_product_benchmarks.cpp` call the resulting functions. The focused
layout comparison is in `native/lispb/kernel/benchmarks/dot_product_3d_benchmarks.cpp`. The semantic
declarations in `Plugins/SandboxCore/Source/SandboxCore/Kernels/candidate_math.lispb` and the
renderer in `native/lispb/kernel/src/avx2_lab_renderer.cpp` are the committed sources of truth.
...
The central project manifest explicitly orders Kernel input documents and defines named emissions:

```lisp
(kernel sandbox-kernels
  :sources ("array_math.lispb" "candidate_math.lispb")
  (emission kernel-unreal
    :profile unreal
    :output-root (project-path "Plugins/SandboxCore/Source/SandboxCore")))
```

Input paths are relative to the project root and must use the `.lispb` extension.

```text
lispb <generate|check|validate> --project lispb/project.lispb --target <kernel-emission>
...
out/build/kernel-benchmark/native/lispb/kernel/kernel-native-benchmarks.exe --benchmark_repetitions=5 --benchmark_enable_random_interleaving=true
```
