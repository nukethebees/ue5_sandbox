...
```lisp
(include "Experiments/Patterns.lispb")

(widget-class SRadar3DWidget
  (image-builder image_))
```

Configure search roots on the Slate target in `lispb/project.lispb`:

```lisp
(slate radar
  :include-directories ("../Widgets")
  :sources ("Radar3DShowcase.lispb")
...
The `codegen` workflow also generates and builds `tests/compile/widgets.lispb` into a standalone
...
```text
lispb <generate|check|validate|expand> --project lispb/project.lispb --target <slate-target>
```
...
only DSL text, so the output can be redirected to a `.lispb` file. Comments and original
...
```text
out/build/codegen/native/lispb/lispb.exe expand --target slate-radar
```

Input and output paths are resolved by `lispb/project.lispb`. Output filenames are derived from
widget class declarations rather than listed in the project manifest.
