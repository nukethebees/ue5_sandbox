...
template <int32 Capacity>
    requires (Capacity >= 0)
struct TVectors3Storage {
    using View = FVectors3fView;
    using ConstView = FVectors3fConstView;

    auto get_view(int32 const offset, int32 const count) -> View {
        return View{TArrayView<float>{xs_.data() + offset, count},
                    TArrayView<float>{ys_.data() + offset, count},
                    TArrayView<float>{zs_.data() + offset, count}};
    }
    auto get_const_view(int32 const offset, int32 const count) const -> ConstView {
        return ConstView{TConstArrayView<float>{xs_.data() + offset, count},
                         TConstArrayView<float>{ys_.data() + offset, count},
                         TConstArrayView<float>{zs_.data() + offset, count}};
    }
    ...
    void move_construct_at(int32 const index, TVectors3Storage& other, int32 const other_index) {
    ...
    void move_assign_at(int32 const dst_index, TVectors3Storage& other, int32 const source_index) {
...
struct TFixedVectors3 {
    ...
    inline static constexpr bool supports_copy_construction{std::is_copy_constructible_v<float>};
    inline static constexpr bool supports_move_construction{std::is_move_constructible_v<float>};
    ...
    inline static constexpr bool supports_uninitialised_storage{
        std::is_trivially_copyable_v<float> && std::is_trivially_destructible_v<float>};
    template <typename TArg0, typename TArg1, typename TArg2>
    inline static constexpr bool supports_element_construction_from{
        std::is_constructible_v<float, TArg0&&> && std::is_constructible_v<float, TArg1&&> &&
        std::is_constructible_v<float, TArg2&&>};

    TFixedVectors3() noexcept = default;
    TFixedVectors3(TFixedVectors3 const& other)
    ...
    TFixedVectors3(TFixedVectors3 const&)
        requires (!supports_copy_construction)
    = delete;
    TFixedVectors3(TFixedVectors3&& other) noexcept(supports_nothrow_move_construction)
    ...
    TFixedVectors3(TFixedVectors3&&)
        requires (!supports_move_construction)
    = delete;
    ~TFixedVectors3() { reset(); }

    auto operator=(TFixedVectors3 const& other) -> TFixedVectors3&
        requires supports_copy_construction
    {
    ...
    auto operator=(TFixedVectors3 const&) -> TFixedVectors3&
        requires (!supports_copy_construction)
    = delete;
    auto operator=(TFixedVectors3&& other) noexcept(supports_nothrow_move_construction)
        -> TFixedVectors3&
        requires supports_move_construction
    {
        if (this != std::addressof(other)) {
    ...
    auto operator=(TFixedVectors3&&) -> TFixedVectors3&
        requires (!supports_move_construction)
    = delete;
    ...
    void set(int32 const i, float const x, float const y, float const z) {
        get_view().set(i, x, y, z);
    }
    ...
    auto emplace_back(TArg0&& new_xs, TArg1&& new_ys, TArg2&& new_zs) -> size_type {
        ...
        storage_.construct_at(index,
                              std::forward<TArg0>(new_xs),
                              std::forward<TArg1>(new_ys),
                              std::forward<TArg2>(new_zs));
        ++size_;
        return index;
    }
    template <typename TArg0, typename TArg1, typename TArg2>
    ...
    auto add(TArg0&& new_xs, TArg1&& new_ys, TArg2&& new_zs) -> size_type {
        return emplace_back(
            std::forward<TArg0>(new_xs), std::forward<TArg1>(new_ys), std::forward<TArg2>(new_zs));
    }
  ...
  private:
    ...
    TVectors3Storage<Capacity> storage_;
    size_type size_{};
};
} // namespace codegen_compile_fixture
