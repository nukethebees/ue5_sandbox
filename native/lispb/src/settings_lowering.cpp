...
auto title_case(std::string const& value) -> std::string {
    ...
    auto flush_token = [&]() {
        ...
        if (token == "aa") {
            ...
            token.front() =
                static_cast<char>(std::toupper(static_cast<unsigned char>(token.front())));
            result << token;
        }
        token.clear();
        capitalize = true;
...
auto build_header(SettingsModuleSchema const& module,
    ...
    output << "\n    auto operator==(" << module.state_name
           << " const&) const -> bool = default;\n";
    output << "};\n\n";
    ...
    output << "private:\n"
           ...
           << "    auto derived() const -> Derived const& { return static_cast<Derived "
              "const&>(*this); }\n"
           << "};\n";
...
auto build_source(SettingsModuleSchema const& module, std::map<std::string, CppType> const& types)
    ...
    output << "auto game_setting_descriptors() -> TConstArrayView<FGameSettingDescriptor> {\n"
           << "    return descriptors;\n}\n\n"
           << "auto game_setting_descriptor(EGameSetting const id) -> FGameSettingDescriptor "
              "const& {\n"
