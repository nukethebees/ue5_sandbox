...
class TQuantizedDecimal {
  ...
  private:
    ...
    // The integer stores the floor. The fraction is always non-negative, so -0.42 is stored as
    // integer -1 and fraction 58 rather than integer 0 and a signed fraction.
