...
auto collect_errors(::ioj::sim::levels::LevelDefinition const& definition)
    ...
    if (!definition.unlock_level_ids.empty()) {
        errors.emplace_back("Editor level source does not support unlock criteria");
    }
    if (!definition.mission_events.empty()) {
        errors.emplace_back("Editor level source does not support mission events");
    }
...
void append_id_clause(std::string& source,
...
auto emit_editor_level_source(::ioj::sim::levels::LevelDefinition const& definition)
    ...
    std::string source{
        ";; Generated by Space Game Level Authoring. Manual edits are supported; saving rewrites "
        "canonical explicit S7.\n\n(level\n"};
    ...
    if (!definition.player_entity_id.empty()) {
        ...
        source += "\n  ;; Observer camera for this playerless level.\n";
        source += "  (camera\n    (look-at";
    ...
    if (definition.mission) {
        ...
        source += "\n  (mission\n";
        source += "    (mode '" + std::string{mission_mode_symbol(mission.mode)} + ")\n";
        if (mission.time_limit_seconds) {
            source += "    (time-limit " + format_number(*mission.time_limit_seconds) + ")\n";
        }
        if (mission.kill_count) {
            source += "    (kill-count " + std::to_string(*mission.kill_count) + ")\n";
        }
        append_id_clause(source, "heroes", mission.hero_entity_ids);
        append_id_clause(source, "must-survive", mission.must_survive_entity_ids);
        append_id_clause(source, "required-kills", mission.required_kill_entity_ids);
        source.back() = ')';
        source += '\n';
    }
