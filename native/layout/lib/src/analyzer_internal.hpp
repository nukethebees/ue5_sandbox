...
class AggregateLayoutAnalyzer {
  public:
    [[maybe_unused]] AggregateLayoutAnalyzer(lispb::schema::TypeGraph const& types,
                                             AbiProfile const& abi)
        : types_{types}
        , abi_{abi} {}

    [[maybe_unused]] auto analyze_record(lispb::schema::TypeId const type) -> RecordAnalysis {
        std::vector<lispb::schema::TypeId> active;
        return analyze_record(type, active);
    }

    [[maybe_unused]] auto analyze_union(lispb::schema::TypeId const type) -> UnionAnalysis {
        std::vector<lispb::schema::TypeId> active;
        return analyze_union(type, active);
    }

    [[maybe_unused]] auto analyze_tagged_union(lispb::schema::TypeId const type)
        -> TaggedUnionAnalysis {
        std::vector<lispb::schema::TypeId> active;
