...
bool validate_entry(FWidgetBlueprintGenerationEntry const& entry,
    ...
    if (widget_class == nullptr) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d has no widget class."),
               entry_index);
        return false;
    }
    ...
    if (!widget_class->IsChildOf(UUserWidget::StaticClass())) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d class '%s' does not derive from UUserWidget."),
               entry_index,
    ...
    if (!try_make_asset_paths(entry, package_path, object_path, path_error)) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d has an invalid output path: %s"),
               entry_index,
               *path_error.ToString());
        return false;
    }
...
bool delete_existing_asset(FWidgetBlueprintGenerationEntry& entry,
    ...
    if (!asset_data.IsValid()) {
        UE_LOG(LogSandboxEditor,
               Log,
               TEXT("Widget generation row %d has no existing asset at '%s'."),
               entry_index,
    ...
    UE_LOG(LogSandboxEditor,
           Log,
           TEXT("Widget generation row %d is deleting existing asset '%s'."),
           entry_index,
           *object_path.ToString());
    ...
    if (deleted_asset_count != assets_to_delete.Num() ||
        ...
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d could not delete existing asset '%s'."),
               entry_index,
               *object_path.ToString());
        return false;
    }
...
bool add_bind_widgets(UWidgetBlueprint& widget_blueprint,
    ...
    for (TFieldIterator<FProperty> property_iterator(&widget_class,
        ...
        if (object_property == nullptr || object_property->PropertyClass == nullptr ||
            ...
            UE_LOG(LogSandboxEditor,
                   Warning,
                   TEXT("Widget generation row %d skipped binding '%s': property is not a UWidget "
                        "object property."),
                   entry_index,
                   *property->GetName());
            continue;
        ...
        if (is_project_widget_class(*required_widget_class)) {
            ...
            if (!mapped_widget_class) {
                UE_LOG(LogSandboxEditor,
                       Error,
                       TEXT("Widget generation row %d requires a configured WBP mapping for "
                            "binding '%s' (%s)."),
                       entry_index,
                       *property->GetName(),
        ...
        if (widget_class_to_construct->HasAnyClassFlags(CLASS_Abstract | CLASS_Deprecated)) {
            UE_LOG(LogSandboxEditor,
                   Warning,
                   TEXT("Widget generation row %d skipped binding '%s': widget class '%s' cannot "
                        "be instantiated."),
                   entry_index,
                   *property->GetName(),
        ...
        if (generated_widget == nullptr) {
            UE_LOG(LogSandboxEditor,
                   Error,
                   TEXT("Widget generation row %d could not construct binding '%s' as '%s'."),
                   entry_index,
                   *property->GetName(),
                   *widget_class_to_construct->GetPathName());
            if (is_project_widget_class(*required_widget_class)) {
        ...
        if (root_canvas.AddChild(generated_widget) == nullptr) {
            UE_LOG(LogSandboxEditor,
                   Warning,
                   TEXT("Widget generation row %d could not attach binding '%s' to RootCanvas."),
                   entry_index,
                   *property->GetName());
            if (is_project_widget_class(*required_widget_class)) {
                return false;
        ...
        UE_LOG(LogSandboxEditor,
               Log,
               TEXT("Widget generation row %d added binding '%s' as '%s'."),
               entry_index,
               *property->GetName(),
...
UWidgetBlueprint* create_widget_blueprint(FWidgetBlueprintGenerationEntry const& entry,
    ...
    UE_LOG(LogSandboxEditor,
           Log,
           TEXT("Widget generation row %d is creating Widget Blueprint '%s'."),
           entry_index,
           *package_path);
    ...
    if (package == nullptr) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d could not create package '%s'."),
               entry_index,
               *package_path);
        return nullptr;
    }
    ...
    if (widget_blueprint == nullptr || widget_blueprint->WidgetTree == nullptr) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d could not create Widget Blueprint '%s'."),
               entry_index,
               *package_path);
        return nullptr;
    }
    ...
    if (root_canvas == nullptr) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d could not create RootCanvas for '%s'."),
               entry_index,
               *package_path);
    ...
    UE_LOG(
        LogSandboxEditor, Log, TEXT("Widget generation row %d created RootCanvas."), entry_index);
    if (!add_bind_widgets(
            *widget_blueprint, *root_canvas, *entry.widget_class.Get(), entry_index)) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d failed while adding BindWidget children."),
               entry_index);
        package->MarkPackageDirty();
        return nullptr;
    }
    ...
    if (compiler_results.NumErrors > 0 || widget_blueprint->Status == BS_Error) {
        UE_LOG(LogSandboxEditor,
               Error,
               TEXT("Widget generation row %d failed to compile '%s' with %d error(s)."),
               entry_index,
               *package_path,
    ...
    UE_LOG(LogSandboxEditor,
           Log,
           TEXT("Widget generation row %d compiled '%s' with %d warning(s)."),
           entry_index,
           *package_path,
           compiler_results.NumWarnings);
    UE_LOG(LogSandboxEditor,
           Log,
           TEXT("Generated Widget Blueprint '%s' from '%s'."),
           *package_path,
...
void UWidgetBlueprintGenerationDataAsset::generate_widgets() {
    ...
    if (selected_widget_count == 0) {
        ...
        UE_LOG(LogSandboxEditor,
               Warning,
               TEXT("Widget generation was requested, but no rows are selected."));
        return;
    }

    UE_LOG(LogSandboxEditor,
           Log,
           TEXT("Preparing to generate %d Widget Blueprint row(s)."),
           selected_widget_count);
    ...
    for (int32 widget_index{0}; widget_index < widget_count; ++widget_index) {
        ...
        UE_LOG(LogSandboxEditor,
               Log,
               TEXT("Preparing widget generation row %d for '%s'."),
               widget_index,
        ...
        UE_LOG(LogSandboxEditor,
               Log,
               TEXT("Widget generation row %d completed successfully."),
               widget_index);
    }

    refresh_existing_widgets();
    MarkPackageDirty();
    UE_LOG(LogSandboxEditor, Log, TEXT("Widget Blueprint generation finished."));
}
