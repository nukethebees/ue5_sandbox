...
class SkillGenerator:
    ...
    def init_generated_file(self, name:str) -> FileBuffer:
        return FileBuffer(self.generated_dir / name, "")
...
"""
    ...
    def write_skills_enum(self) -> None:
        ...
        self.header += f"""UENUM(BlueprintType)
...
"""
            ...
            self.header += f"inline constexpr std::array<{self.player_skills_enum}, {n}> {lname}_values{{{{\n"
            first = True
    ...
    def write_get_display_name(self) -> None:
        self.header += f"""inline auto get_display_name({self.player_skills_enum} value) -> FName {{
    switch (value) {{
"""
        for skill in self.skills:
            self.header += f"""        case {skill.config.get_full_enum_value()}: {{
...
"""
    def write_get_display_string(self) -> None:
        self.header += f"""inline auto get_display_string({self.player_skills_enum} value) -> FString const& {{
    switch (value) {{
"""
        for skill in self.skills:
            self.header += f"""        case {skill.config.get_full_enum_value()}: {{
...
"""
    def write_get_display_string_view(self) -> None:
        self.header += f"""inline auto get_display_string_view({self.player_skills_enum} value) -> TStringView<TCHAR> {{
...
"""
    ...
    def write_detail_customisation(self) -> None:
        struct_name = f"F{self.player_skills_struct.rawname()}PropDisplay"
