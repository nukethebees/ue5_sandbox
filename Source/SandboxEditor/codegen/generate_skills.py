...
class SkillGenerator:
    ...
    player_skill_struct = UnrealType("PlayerSkill", UnrealType.Tag.STRUCT)
...
"""
    ...
    def write_struct_skill_enum_template_accessors(self) -> None:
        ...
        self.output_file += f"""
    template <{self.player_skills_enum} {key}>
    ...
    constexpr auto get(this auto const& self, 
                       {self.player_skills_enum} {key}) -> {self.skill_value_typename} {{
        switch ({key}) {{"""
        for skill in self.skills:
            self.output_file += f"""
