...
void FTestBatchOrchestratorLevelSetup::setup(
    ...
    command_builder.Do([this] {
        ...
        if (!legacy_checks->not_nullptr(loaded_config, TEXT("Default test simulation config loads")) ||
            !legacy_checks->is_true(loaded_config->is_valid(), TEXT("Default test simulation config is valid"))) {
            return;
        }
...
auto FTestBatchOrchestratorLevelSetup::construct_level() -> bool {
    ...
    map_directory = FPaths::Combine(FPaths::ProjectContentDir(), TEXT("SandboxReusableTestLevels"),
                                    FGuid::NewGuid().ToString(EGuidFormats::Digits));
    map_name = TEXT("level");
    auto const package_name{FPackageName::FilenameToLongPackageName(FPaths::Combine(map_directory, map_name))};
