...
TEST_CLASS(S7LevelDefinitionWriter, "Sandbox.UnitTests")
{
    ...
    {
        auto const source{ml::s7::emit_editor_level_source(make_player_level())};
        if (!TestRunner->TestTrue(TEXT("Source is emitted"), source.has_value())) {
            TestRunner->AddError(source.error());
            return;
        }

        constexpr TCHAR expected[]{
            LR"(;; Generated by Space Game Level Authoring. Manual edits are supported; saving rewrites canonical explicit S7.
...
(level
  ...
  (entities
    ...
    {
        auto const first{ml::s7::emit_editor_level_source(make_player_level(false))};
        auto const second{ml::s7::emit_editor_level_source(make_player_level(true))};
    ...
    TEST_METHOD(ExpandsLargeProceduralLevelIntoExplicitSemanticEquivalent)
    {
        ...
        auto const source{ml::s7::emit_editor_level_source(input.definition.GetValue())};
