...
void run_worldless_turret_search_requires_line_of_sight(FAutomationTestBase& test,
    ...
    checks.are_equal(
        3, static_cast<int32>(targets.size()), TEXT("All turret targets are available"));
    if (static_cast<int32>(targets.size()) != 3) {
        return;
    }
    ...
    if (targets[0].is_valid()) {
        checks.dist_zero(FVector3f{1000.f, 1000.f, 0.f},
                         ml::to_unreal(harness.get_registry().get_location(targets[0])),
                         1.f,
