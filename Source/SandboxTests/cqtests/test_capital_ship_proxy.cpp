...
TEST_CLASS(TestCapitalShipProxy, "Sandbox.FunctionalTests")
{
                          ...
                          *TestRunner,
                          [this](UWorld& world, UTestSimulationConfig const& config) {
                              auto* const capital_config{
                                  config.simulation_config->capital_ships_config.Get()};
    ...
    TEST_METHOD(SpawnsWithConfiguredHealth)
    {
        TestCommandBuilder.Do([this] {
            ...
            if (!TestRunner->TestEqual(TEXT("One capital ship is spawned from the proxy"),
            ...
            TestRunner->TestEqual(
                TEXT("Capital ship health matches its config"), health, expected_health);
        });
    }
};
