...
TEST_CLASS(SimulationConfig, "Sandbox.UnitTests")
{
    TEST_METHOD(DeepCopy)
    {
        ...
        if (!TestRunner->TestTrue(TEXT("Default simulation config is valid"),
                                  source->is_valid())) {
            return;
        }
        ...
        auto verify_child{[this, copy](auto const* const source_child,
            ...
            if (!TestRunner->TestNotNull(
                    *FString::Printf(TEXT("%s source exists"), child_name), source_child)) {
                return;
            }
            if (!TestRunner->TestNotNull(
                    *FString::Printf(TEXT("%s copy exists"), child_name), copied_child)) {
                return;
            }

            TestRunner->TestTrue(
                *FString::Printf(TEXT("%s has a different address"), child_name),
                source_child != copied_child);
            TestRunner->TestTrue(*FString::Printf(TEXT("%s has the copied bundle as its outer"),
                                                  child_name),
        ...
        verify_child(
            source->lasers_config.Get(), copy->lasers_config.Get(), TEXT("Lasers config"));
        ...
        TestRunner->TestTrue(
            TEXT("Player ship nested assets remain shared"),
