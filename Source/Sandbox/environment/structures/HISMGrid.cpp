...
void AHISMGrid::OnConstruction(FTransform const& transform) {
    Super::OnConstruction(transform);
    update_isms(false);
}

auto AHISMGrid::make_index(int32 x, int32 y, int32 z) -> FVector3d {
    return {
...
void AHISMGrid::update_isms_button() {
    return update_isms(true);
}
void AHISMGrid::update_isms(bool warn_on_no_mesh) {
...
auto AHISMGrid::add_element(FVector3d index,
                            FVector3d origin,
