...
void PlannerUi::draw_properties_panel() {
    ...
    if (auto const* enumeration{std::get_if<EnumType>(&node.definition)}) {
        ...
        auto const& source{analysis_session_.inputs.workspace.types().type(quantized->source.type)};
        ...
        auto const& source{analysis_session_.inputs.workspace.types().type(optional->source.type)};
        ImGui::TextUnformatted("Semantic source");
        ImGui::SameLine();
        if (ImGui::SmallButton(source.cpp_spelling.c_str())) {
                ...
                for (auto const& field : analysis.fields) {
                    ...
                    if (field.semantic_type.has_value()) {
                        auto const& semantic_type{
                            analysis_session_.inputs.workspace.types().type(*field.semantic_type)};
