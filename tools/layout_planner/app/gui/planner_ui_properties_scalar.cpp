...
void PlannerUi::draw_enum_editor(TypeNode const& node, EnumType const&) {
    ...
    if (detail::section("Generation policy")) {
        ...
        if (ImGui::BeginCombo("##Reflection",
            ...
            for (std::size_t index{}; index < enum_reflections.size(); ++index) {
                ...
                if (ImGui::Selectable(codegen::enum_reflection_name(reflection).data(),
                    ...
                    ImGui::EndCombo();
                    static_cast<void>(apply_document_edit(ReplaceEnum{
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "Requires a valid final count-sentinel enumerator and implicit values.");
        }
    ...
    if (detail::section("Unreal projection", false)) {
        ...
        auto edit_projection_text = [&](char const* label, auto& buffer, auto&& assign) {
            ...
            auto const submitted{ImGui::InputText(input_id.c_str(),
                                                  buffer.data(),
                                                  buffer.size(),
                                                  ImGuiInputTextFlags_EnterReturnsTrue)};
        ...
        edit_projection_text("Generated header path",
                             enum_projection_header_,
        ...
        enum_projection_reflection_ =
            std::clamp(enum_projection_reflection_,
                       0,
                       static_cast<int>(enum_projection_reflections.size() - 1));
        ...
        if (ImGui::BeginCombo(
            ...
            for (std::size_t index{}; index < enum_projection_reflections.size(); ++index) {
                ...
                auto const selected_reflection{enum_projection_reflection_ ==
        ...
        ImGui::PushTextWrapPos(0.0F);
        ImGui::TextDisabled(
            "Projection files are generated consumer outputs; semantic enum width and "
    ...
    if (detail::section("Enumerators")) {
        if (ImGui::Button("+ Enumerator")) {
            ...
            do {
                ...
                std::ranges::find(replacement.values, name, &codegen::EnumeratorSchema::name) !=
        ...
        if (ImGui::Button("Duplicate")) {
            ...
            while (std::ranges::find(
                       replacement.values, copy.name, &codegen::EnumeratorSchema::name) !=
                   replacement.values.end()) {
        ...
        if (detail::begin_editable_table("enumerators", 9, schema->values.size())) {
            ...
            for (std::size_t index{}; index < schema->values.size(); ++index) {
                ...
                auto draw_optional_editor =
                    [&](char const* label,
                        auto& buffer,
                        std::optional<std::string> codegen::EnumeratorSchema::* member) {
                        ImGui::TableNextColumn();
                        if (row_selected) {
                            ImGui::SetNextItemWidth(-1.0F);
                ...
                draw_optional_editor("##display",
                                     enum_value_display_name_,
                                     &codegen::EnumeratorSchema::display_name);
    ...
    if (detail::section("Semantic relationship", false)) {
        ...
        if (ImGui::BeginCombo("##relationship-kind",
            ...
            for (std::size_t kind_index{}; kind_index < semantic_relationship_kinds.size();
                ...
                auto const chosen{integer_scalar_relationship_kind_ ==
                                  static_cast<int>(kind_index)};
                ...
                for (std::size_t unit_index{}; unit_index < semantic_relationship_units.size();
                    ...
                    if (ImGui::Selectable(codegen::semantic_relation_unit_name(unit).data(),
                        ...
                        if (schema->relationship.has_value()) {
                            ...
                            if (apply_document_edit(
                                    ReplaceIntegerScalar{.declaration = *declaration,
                                                         .schema = std::move(replacement)})) {
                                integer_scalar_editor_declaration_.reset();
                                ImGui::EndCombo();
                                return true;
    ...
    if (detail::section("Named codes")) {
        ...
        if (ImGui::Button("Delete")) {
            ...
            replacement.named_codes.erase(replacement.named_codes.begin() +
                                          static_cast<std::ptrdiff_t>(*selected_index));
            auto const next_code{replacement.named_codes.empty()
                                     ? std::string{}
                                     : replacement
                                           .named_codes[std::min(
                                               *selected_index, replacement.named_codes.size() - 1)]
                                           .name};
            if (apply_document_edit(ReplaceIntegerScalar{.declaration = *declaration,
                                                         .schema = std::move(replacement)})) {
            ...
            for (std::size_t index{}; index < schema->named_codes.size(); ++index) {
                ...
                if (ImGui::BeginDragDropTarget()) {
                    if (auto const* payload{
                            ImGui::AcceptDragDropPayload("INTEGER_SCALAR_CODE_ROW")}) {
                        auto const source_index{*static_cast<std::size_t const*>(payload->Data)};
...
auto PlannerUi::draw_fixed_point_editor(TypeNode const& node, FixedPointType const&) -> bool {
    ...
    if (fixed_point_editor_declaration_ != declaration) {
        std::snprintf(fixed_point_minimum_.data(),
                      fixed_point_minimum_.size(),
                      "%s",
                      schema->minimum_value.value_or("").c_str());
        std::snprintf(fixed_point_maximum_.data(),
                      fixed_point_maximum_.size(),
                      "%s",
                      schema->maximum_value.value_or("").c_str());
    ...
    auto edit_bound = [&](char const* label, std::array<char, 128>& buffer, bool const minimum) {
        ImGui::SetNextItemWidth(220.0F);
        ...
        auto replacement{*schema};
        auto const value{buffer.front() == '\0' ? std::nullopt
                                                : std::optional{std::string{buffer.data()}}};
        if (minimum) {
            replacement.minimum_value = value;
        } else {
            replacement.maximum_value = value;
        }
        if (apply_document_edit(
                ReplaceFixedPoint{.declaration = *declaration, .schema = std::move(replacement)})) {
            fixed_point_editor_declaration_.reset();
    ...
    if (edit_bound("Minimum value", fixed_point_minimum_, true) ||
        edit_bound("Maximum value", fixed_point_maximum_, false)) {
        return true;
    }
    ImGui::TextDisabled("Leave a bound empty to use the full representable endpoint.");
    return false;
}
