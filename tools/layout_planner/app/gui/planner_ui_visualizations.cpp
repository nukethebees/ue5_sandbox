...
void PlannerUi::draw_packed_layout(PackedType const& packed, PackedAnalysis const& baseline) {
    ...
    if (detail::section("Sequential access set")) {
        ...
        if (analysis_session_.results().packed_access_analysis.has_value()) {
            ...
            if (ImGui::BeginTable("packed-access",
                ...
                draw_access_stat("Read useful bits",
                                 detail::format_number(access.read_useful_bits));
                draw_access_stat("Write useful bits",
                                 detail::format_number(access.write_useful_bits));
                ...
                draw_access_stat("Minimum pages",
    ...
    if (detail::section("Bit layout")) {
        ...
        if (analysis_session_.inputs.selection.packed_access_set_explicit) {
            ...
            auto const selected{std::ranges::find(baseline.fields,
        ...
        auto const* packed_schema{
            declaration.has_value() ? document_->packed_value_schema(*declaration) : nullptr};
        ...
        if (packed_schema != nullptr && baseline.fields.size() > 1) {
            ImGui::TextDisabled("Drag a baseline divider to change both adjacent LispB field "
                                "widths as one undo step.");
        }
        ...
        for (auto const& [variant_id, analysis] : analysis_session_.results().packed_variants) {
            ...
            if (adjustment.has_value() &&
                adjustment->left_field_index + 1 < packed.segments.size()) {
        ...
        if (!analysis_session_.results().packed_variants.empty()) {
            ImGui::TextDisabled(
                "Click a variant to edit it. Drag a divider to transfer whole bits.");
        }
    }
}
...
void PlannerUi::draw_record_layout(RecordAnalysis const& analysis) {
    ...
    if (detail::section("Object layout")) {
        ...
        if (analysis.size_bytes.has_value() && *analysis.size_bytes != 0) {
            ...
            for (std::size_t index{}; index < analysis.members.size(); ++index) {
                ...
                auto const color{selected ? selected_color
                                          : ImVec4{0.48F + 0.07F * static_cast<float>(index % 2),
                                                   0.34F,
                                                   0.18F,
                                                   1.0F}};
                ...
                if (member_width >= 42.0F) {
                    draw_list->AddText({left + 4.0F, origin.y + 5.0F},
                                       ...
                                       member.name.c_str());
                    auto const range{
                        std::to_string(*member.offset_bytes) + ".." +
            ...
            if (ImGui::IsItemHovered()) {
                auto const relative_x{
                    std::clamp(ImGui::GetIO().MousePos.x - origin.x, 0.0F, width)};
                auto const byte{
        ...
        if (ImGui::BeginTable("record-layout",
            ...
            for (auto const& member : analysis.members) {
                ...
                ImGui::TableNextColumn();
                ImGui::TextUnformatted(detail::format_number(member.element_facts.transform(
    ...
    if (detail::section("Sequential access set")) {
        ...
        if (analysis_session_.results().record_access_analysis.has_value()) {
            ...
            if (ImGui::BeginTable("record-member-access",
                ...
                draw_stat("Bytes in touched pages",
                          detail::format_bytes(access.page_bytes_touched));
            ...
            ImGui::TextDisabled(
                ...
                "contiguous AoS base assumed. Footprints describe address coverage and do not "
                "infer "
            ...
            ImGui::TextDisabled(
                "Select record members in the Access column to define the access set.");
        }
...
void PlannerUi::draw_soa_layout(SoaType const& soa, SoaAnalysis const& baseline) {
    ...
    if (detail::section("Analysis scale")) {
        ...
        ImGui::TextDisabled(
            analysis_session_.inputs.soa_allocation_strategy ==
                ...
                : "Columns occupy one alignment-aware block; region coverage assumes a "
                  "region-aligned "
                  "block origin.");
        if (ImGui::BeginTable("soa-columns",
            ...
            for (auto const& column : active.columns) {
                ...
                ImGui::TableNextColumn();
                ImGui::TextUnformatted(
                    detail::format_number(column.elements_per_cache_line).c_str());
                ImGui::TableNextColumn();
    ...
    if (detail::section("Sequential access set")) {
        ...
        if (analysis_session_.results().soa_access_analysis.has_value()) {
            ...
            if (ImGui::BeginTable("soa-column-access",
                ...
                auto const* const region_qualifier_lower{access.footprint_exact ? "exact"
                                                                                : "minimum"};
                draw_stat("Operation", detail::access_operation_summary(access.accesses));
                ...
                draw_stat((std::string{"Non-payload bytes in "} + region_qualifier_lower +
                           " cache footprint")
                              .c_str(),
            ...
            draw_footprint_composition(access.footprint_exact
                                           ? "Exact page footprint composition"
            ...
            ImGui::TextDisabled(
                "Select SoA columns in the Access column to define the access set.");
        }
    }

    if (detail::section("Cache-line / page block map")) {
        if (analysis_session_.inputs.soa_allocation_strategy ==
            SoaAllocationStrategy::separate_columns) {
            ImGui::TextDisabled(
                "A single address map is unavailable because separate allocation origins are "
                "Unknown."
            ...
            ImGui::TextDisabled("The block origin is aligned to the selected target region. "
                                "Boundaries and selected "
                                "prefixes show addresses only; they are not cache traffic or a "
    ...
    if (analysis_session_.results().record_soa_access_comparison.has_value() &&
        ...
        if (detail::section("Equivalent AoS / SoA access")) {
            auto const& comparison{*analysis_session_.results().record_soa_access_comparison};
            auto const& record_identity{analysis_session_.inputs.workspace.types()
                                            .type(soa.equivalent_type->type)
                                            .identity};
            ...
            if (ImGui::BeginTable("record-soa-access-comparison",
                ...
                ImGui::TableSetupColumn(comparison.soa_footprint_exact ? "SoA exact"
                                                                       : "SoA minimum");
                ...
                draw_stat(
                    ...
                    detail::format_fit(
                        comparison.soa_minimum_cache_footprint_capacity.fits_l1_data),
                    "—");
                draw_stat(
                    "Footprint <= L2 cache",
