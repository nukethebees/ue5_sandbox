...
void register_chunk_case(ChunkBackend const& backend,
                         std::int32_t const count,
                         bool const extreme) {
    auto const name{std::string{"add_scaled/elementwise/chunked16/"} +
                    std::string{backend.name} + "/" + (extreme ? "extreme/" : "ordinary/") +
                    "aligned/" + std::to_string(count)};
    benchmark::RegisterBenchmark(name.c_str(),
                                 run_chunk_benchmark,
                                 backend.kernel,
                                 count,
...
auto register_benchmarks() -> bool {
    ...
    constexpr std::array counts{1,   7,    8,     9,     11,     15,     16,
                                ...
                                4096, 16384, 65536, 100000, 262144, 1048576};
    constexpr std::array chunk_counts{8,     11,    16,     32,     256, 4096,
                                      16384, 65536, 100000, 262144, 1048576};
