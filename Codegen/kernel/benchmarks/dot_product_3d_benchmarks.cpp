...
using AosKernel = void (*)(dot::Float3 const*,
                           dot::Float3 const*,
                           float*,
                           std::int32_t) noexcept;
...
struct SoaBuffers {
    explicit SoaBuffers(std::int32_t const count)
        : lhs_x{count}, lhs_y{count}, lhs_z{count}, rhs_x{count}, rhs_y{count}, rhs_z{count},
          out{count} {}
...
struct AosBuffers {
    explicit AosBuffers(std::int32_t const count) : lhs{count}, rhs{count}, out{count} {}
...
auto register_benchmarks() -> bool {
    ...
    for (auto const count : counts) {
        ...
        benchmark::RegisterBenchmark(name.c_str(),
                                     run_aos_benchmark,
                                     static_cast<AosKernel>(
                                         dot::backend::scalar::dot_product_3d),
                                     count,
                                     false)
            ->UseRealTime();
    }
    for (auto const& backend : backends()) {
        for (auto const count : counts) {
            ...
            benchmark::RegisterBenchmark(
                ...
                ->UseRealTime();
            benchmark::RegisterBenchmark(
                (prefix + "soa-chunked16" + suffix).c_str(),
                run_chunk_benchmark,
                backend.chunk,
                count,
                backend.requires_avx512)
