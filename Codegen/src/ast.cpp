...
constexpr auto generated_file_warning =
...
auto render_signature(Function const& function) -> std::string {
    ...
    for (auto const& parameter : spec.parameters) {
        parameters.push_back(render_parameter(parameter, function.declaration || function.is_header));
    }
...
CppType::CppType(char const* value) : spelling{value} {}
CppType::CppType(std::string value) : spelling{std::move(value)} {}
...
CppType::CppType(std::string value, std::vector<TypeDependency> type_dependencies)
    : spelling{std::move(value)}, dependencies{std::move(type_dependencies)} {}
...
Member::Member(CppType value_type, std::string value_name)
    : type{std::move(value_type)}, name{std::move(value_name)} {}
    ...
    return std::visit(
        [&](auto const& value) -> std::string {
            ...
            if constexpr (std::is_same_v<T, Raw>) {
                ...
                return context.apply_indent("#include " + std::string(1, left) + value.path + right);
