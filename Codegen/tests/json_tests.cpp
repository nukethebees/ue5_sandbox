...
TEST(Json, LoadsTypedSoaManifest) {
    ...
    files.write(
        "types.json",
        R"({"types":{"handle":{"spelling":"FHandle","header":"Handle.h","pass_by":"value","operations":{"add_element":{"function":"add","pass_by":"value"},"remove_at_swap":"remove_at_swap","set_element":{"function":"set","pass_by":"value"}}}}})");
    files.write(
        "modules.json",
    ...
    EXPECT_EQ(manifest.types.at("handle").operation(TypeOperation::remove_at_swap),
              "remove_at_swap");
    EXPECT_EQ(manifest.types.at("handle").operation(TypeOperation::set_element), "set");
    EXPECT_EQ(manifest.types.at("handle").parameter_passing, ParameterPassing::value);
    EXPECT_EQ(manifest.types.at("handle").operation_parameter_passing(TypeOperation::add_element),
              ParameterPassing::value);
    EXPECT_EQ(manifest.types.at("handle").operation_parameter_passing(TypeOperation::set_element),
              ParameterPassing::value);
...
TEST(Json, LoadsSettingsModules) {
    TemporaryManifest files;
    files.write("types.json", R"({"types":{}})");
    files.write(
        "modules.json",
        R"({"modules":[{"kind":"settings","name":"settings","header":"Settings.h","source":"Settings.cpp","api_name":"TSettingsAccess","state_name":"FSettingsState","categories":[{"name":"video","label":"Video"}],"settings":[{"name":"vsync","label":"VSync","category":"video","value_type":"bool","backend":"engine","apply":"deferred","control":{"kind":"toggle"}}]}]})");
    files.write("manifest.json",
                R"({"schema_version":11,"types":"types.json","modules":["modules.json"]})");
    ...
    EXPECT_EQ(module.api_name, "TSettingsAccess");
...
TEST(Json, RejectsMalformedDocumentsWithTheirFileName) {
    TemporaryManifest files;
    files.write("manifest.json", R"({"schema_version":7,)");
...
TEST(Json, RejectsDuplicateFieldsInsteadOfSilentlyOverwritingThem) {
    TemporaryManifest files;
    files.write("manifest.json",
                R"({"schema_version":11,"schema_version":11,"types":"types.json","modules":[]})");
    ...
    files.write("manifest.json", R"({"schema_version":11,"types":"types.json","modules":[]})");
...
TEST(Json, ReportsMissingRequiredFields) {
    TemporaryManifest files;
    files.write("types.json", R"({"types":{}})");
    files.write("manifest.json", R"({"schema_version":11,"types":"types.json"})");
...
TEST(Json, ReportsMissingReferencedDocuments) {
    ...
    files.write("types.json", R"({"types":{}})");
    files.write("manifest.json",
                R"({"schema_version":11,"types":"types.json","modules":["missing.json"]})");
...
TEST(Json, LoadsEnumModulesAndConversions) {
    ...
    files.write(
        "modules.json",
        R"({"modules":[{"kind":"enum","name":"modes","header":"Modes.h","source":"Modes.cpp","helper_namespace":"ml","enums":[{"name":"EMode","underlying_type":"uint8","reflection":"blueprint","enum_array":false,"export_specifier":"PROJECT_API","values":[{"name":"First","serialized_name":"first"},{"name":"Readable","value":"7","display_name":"Readable Value","serialized_name":"readable"},{"name":"COUNT","hidden":true,"serialized_name":"count"}],"conversions":["lex_to_string","string_view","string","lex_to_display_string","display_string_view","display_string","lex_to_serialized_string","try_parse_serialized"]}]}]})");
    files.write("manifest.json",
                R"({"schema_version":11,"types":"types.json","modules":["modules.json"]})");
    ...
    EXPECT_EQ(schema.values[1].serialized_name, "readable");
    EXPECT_TRUE(schema.values[2].hidden);
    EXPECT_EQ(schema.conversions.size(), 8);
}
...
TEST(Json, RejectsUnsupportedSchemaVersionsWithTheirPath) {
    TemporaryManifest files;
    files.write("manifest.json", R"({"schema_version":12,"types":"types.json","modules":[]})");
