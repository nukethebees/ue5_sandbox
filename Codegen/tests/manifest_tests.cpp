  ...
  public:
    TemporaryManifest() {
        ...
        directory_ = std::filesystem::temp_directory_path() /
                     ("sandbox-codegen-manifest-test-" + std::to_string(++sequence));
    ...
    void write_root(std::string const& modules) const {
        write("types.sbxgen", "");
        write("modules.sbxgen", modules);
        write("manifest.sbxgen",
              "(codegen-manifest :schema-version 11 :types \"types.sbxgen\" "
              ":modules (\"modules.sbxgen\"))");
    }
  private:
...
(soa-module example
  ...
  (struct FData
    ...
    files.write(
        "manifest.sbxgen",
        R"((codegen-manifest :schema-version 11 :types "types.sbxgen" :modules ("modules.sbxgen")))");

    auto const manifest{load_manifest(files.path("manifest.sbxgen"))};
...
TEST(Manifest, ReportsSourceLocationForUnknownProperties) {
    ...
    try {
        static_cast<void>(load_manifest(files.path("manifest.sbxgen")));
        FAIL() << "Expected manifest error";
    } catch (ManifestError const& error) {
        auto const message{std::string{error.what()}};
        EXPECT_NE(message.find("modules.sbxgen:4:3"), std::string::npos);
...
TEST(Manifest, RejectsDuplicateProperties) {
    ...
    files.write(
        "manifest.sbxgen",
        R"((codegen-manifest :schema-version 11 :schema-version 11 :types "types.sbxgen" :modules ()))");
    EXPECT_THROW(load_manifest(files.path("manifest.sbxgen")), ManifestError);
}

TEST(Manifest, RejectsMalformedDocumentsWithTheirFileName) {
    TemporaryManifest files;
    files.write("manifest.sbxgen", "(codegen-manifest");
    try {
        static_cast<void>(load_manifest(files.path("manifest.sbxgen")));
        FAIL() << "Expected manifest error";
    } catch (ManifestError const& error) {
        EXPECT_NE(std::string{error.what()}.find("manifest.sbxgen"), std::string::npos);
    }
}

TEST(Manifest, RejectsUnsupportedSchemaVersions) {
    TemporaryManifest files;
    files.write("manifest.sbxgen",
                R"((codegen-manifest :schema-version 12 :types "types.sbxgen" :modules ()))");
    EXPECT_THROW(load_manifest(files.path("manifest.sbxgen")), ManifestError);
}
...
TEST(Manifest, ExplainsVersionOneFunctionQualifierMigration) {
    TemporaryManifest files;
    files.write("manifest.sbxgen",
                R"((codegen-manifest :schema-version 1 :types "types.sbxgen" :modules ()))");
    try {
        static_cast<void>(load_manifest(files.path("manifest.sbxgen")));
        FAIL() << "Expected manifest error";
    } catch (ManifestError const& error) {
...
TEST(Manifest, RejectsMissingReferencedDocuments) {
    TemporaryManifest files;
    files.write("manifest.sbxgen",
                R"((codegen-manifest :schema-version 11 :types "missing.sbxgen" :modules ()))");
    EXPECT_THROW(load_manifest(files.path("manifest.sbxgen")), ManifestError);
}
