...
The general Codegen tool reads S-expression schemas and generates the committed C++ files listed
in `.sandbox-codegen-outputs`. The root `manifest.sbxgen` explicitly orders the shared type file and
module files; paths are relative to the manifest.

```lisp
(codegen-manifest
  :schema-version 11
  :types "types.sbxgen"
  :modules ("core_layouts.sbxgen" "enums.sbxgen"))
```
...
Shared types are top-level declarations in `types.sbxgen`:
...
Generate or check the committed files from the repository root:
