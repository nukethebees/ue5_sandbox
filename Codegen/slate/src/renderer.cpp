...
class Renderer {
  public:
    ...
    auto render(WidgetDeclaration const& widget_declaration) -> std::string {
        ...
        append_line(0, {});
        append_line(0, library ? "namespace SlateGenerated::" + widget_declaration.name + " {"
  ...
  private:
    ...
    void render_function(SlateFunction const& function, bool const library) {
        ...
        for (auto const& binding : function.bindings) {
            ...
            append_line(indent + 4, "auto const " + binding.name + "{" + render_binding(binding) + "};");
        }
    ...
    static auto render_value(Value const& value) -> std::string {
        ...
        if (value.kind == ValueKind::localized_text) {
            ...
            return "NSLOCTEXT(\"" + escape_cpp_string(localized.context) + "\", \"" +
                   escape_cpp_string(localized.key) + "\", \"" +
                   escape_cpp_string(localized.text) + "\")";
        }
        return value.text;
    }
