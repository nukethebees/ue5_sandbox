...
void benchmark_scatter_3d_rdg(TConstArrayView<FScatter3DPoint> const points,
    ...
    FScatter3DRenderer renderer;
    for (int32 iteration{0}; iteration < warmup_iterations; ++iteration) {
        renderer.render(points, output_resource);
    }
    ...
    for (int32 iteration{0}; iteration < warmup_iterations; ++iteration) {
        TArray<FScatter3DPoint> snapshot{points};
        ENQUEUE_RENDER_COMMAND(WarmupScatter3DGPU)
        ([points_copy = MoveTemp(snapshot),
          output_resource](FRHICommandListImmediate& rhi_command_list) mutable {
            (void)measure_scatter_3d_gpu(rhi_command_list, MoveTemp(points_copy), output_resource);
        });
    }
    FlushRenderingCommands();
    ...
    for (int32 iteration{0}; iteration < measured_iterations; ++iteration) {
        TArray<FScatter3DPoint> snapshot{points};
        ENQUEUE_RENDER_COMMAND(MeasureScatter3DGPU)
        ([points_copy = MoveTemp(snapshot), output_resource, &gpu_samples](
             FRHICommandListImmediate& rhi_command_list) mutable {
            auto const measurement{
                measure_scatter_3d_gpu(rhi_command_list, MoveTemp(points_copy), output_resource)};
