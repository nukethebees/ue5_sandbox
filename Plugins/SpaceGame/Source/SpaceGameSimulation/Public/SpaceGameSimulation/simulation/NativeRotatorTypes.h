...
inline auto to_native(FRotatorsf::View const view) noexcept -> ::ioj::sim::Rotators3fView {
    auto const count{static_cast<std::size_t>(view.num())};
    return {{view.pitches.GetData(), count},
            {view.yaws.GetData(), count},
            {view.rolls.GetData(), count}};
...
inline auto to_unreal(::ioj::sim::Rotators3fConstView const view) noexcept
