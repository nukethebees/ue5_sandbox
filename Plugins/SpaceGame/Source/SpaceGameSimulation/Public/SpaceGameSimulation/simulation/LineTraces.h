...
namespace ml {
struct FLineTracesView;
struct FLineTracesConstView;

struct SPACEGAMESIMULATION_API FLineTracesConstView {
    ...
    FVectors3f::ConstView starts;
    FVectors3f::ConstView ends;
};

struct SPACEGAMESIMULATION_API FLineTracesView {
    using View = FLineTracesView;
    using ConstView = FLineTracesConstView;
    ...
    FVectors3f::View starts;
    FVectors3f::View ends;
};

struct SPACEGAMESIMULATION_API FLineTraces {
    using View = FLineTracesView;
    using ConstView = FLineTracesConstView;
    ...
    void remove_at_swap(int32 const index, int32 const count, EAllowShrinking const allow_shrinking) {
        starts.remove_at_swap(index, count, allow_shrinking);
        ends.remove_at_swap(index, count, allow_shrinking);
    }
    ...
    void copy_element(int32 const dst_i, Other const& other, int32 const src_i) {
        ml::copy_element(starts, dst_i, other.starts, src_i);
        ml::copy_element(ends, dst_i, other.ends, src_i);
    }

    template <typename Other>
    void copy_elements(int32 const dst_i, Other const& other, int32 const src_i, int32 const count) {
        ml::copy_elements(starts, dst_i, other.starts, src_i, count);
        ml::copy_elements(ends, dst_i, other.ends, src_i, count);
    }
    ...
    void append_from(Other const& other)
        requires ml::SupportsApplyArrayPairsWith<FLineTraces, Other> {
        ml::append_from(starts, other.starts);
        ml::append_from(ends, other.ends);
    }
