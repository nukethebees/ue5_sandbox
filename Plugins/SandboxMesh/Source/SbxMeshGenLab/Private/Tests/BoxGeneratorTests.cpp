...
namespace {
auto is_finite_box_vector(FVector3f const value) -> bool {
    return FMath::IsFinite(value.X) && FMath::IsFinite(value.Y) && FMath::IsFinite(value.Z);
}

auto is_valid_box_uv(FVector2f const value) -> bool {
...
TEST_CLASS(BoxGenerator, "SandboxMesh.UnitTests")
{
    ...
    TEST_METHOD(GeneratesValidIndexedGeometry)
    {
        ...
        for (auto const normal : mesh_data.normals) {
            TestRunner->TestTrue(TEXT("Normal is finite"), is_finite_box_vector(normal));
        ...
        for (auto const uv : mesh_data.uvs) {
            TestRunner->TestTrue(TEXT("UV is finite and in the unit square"), is_valid_box_uv(uv));
        }
    }
