...
TEST_CLASS(CylinderGenerator, "SandboxMesh.UnitTests")
{
    TEST_METHOD(GeneratesExpectedSegmentDependentBuffers)
    {
        FSbxCylinderParameters const parameters{50.0f, 100.0f, 12};
        auto const mesh_data{generate_cylinder(parameters)};

        TestRunner->TestEqual(TEXT("Cylinder has four vertices per segment plus seam vertices"),
                              mesh_data.positions.Num(),
                              parameters.radial_segments * 4 + 4);
        TestRunner->TestEqual(TEXT("Cylinder has four triangles per segment"),
                              mesh_data.indices.Num(),
                              parameters.radial_segments * 12);
        TestRunner->TestEqual(TEXT("Cylinder has a normal per vertex"),
                              mesh_data.normals.Num(),
                              mesh_data.positions.Num());
        TestRunner->TestEqual(
            TEXT("Cylinder has a UV per vertex"), mesh_data.uvs.Num(), mesh_data.positions.Num());
    }

    TEST_METHOD(UsesRequestedRadiusAndHeight)
    {
        auto const mesh_data{generate_cylinder(FSbxCylinderParameters{30.0f, 80.0f, 12})};
    ...
    {
        FSbxCylinderParameters const parameters{30.0f, 80.0f, 12};
        auto const mesh_data{generate_cylinder(parameters)};
        ...
        for (int32 vertex_index{0}; vertex_index < side_vertex_count; ++vertex_index) {
            auto const& position{mesh_data.positions[vertex_index]};
            auto const expected_normal{FVector3f{position.X, position.Y, 0.0f}.GetSafeNormal()};
            TestRunner->TestTrue(TEXT("Side normal points radially outward"),
                                 mesh_data.normals[vertex_index].Equals(expected_normal));
        }

        auto const cap_vertex_count{parameters.radial_segments + 1};
        for (int32 vertex_index{side_vertex_count};
             vertex_index < side_vertex_count + cap_vertex_count;
             ++vertex_index) {
            TestRunner->TestTrue(TEXT("Top cap normal points upward"),
                                 mesh_data.normals[vertex_index].Equals(FVector3f::ZAxisVector));
        }
        for (int32 vertex_index{side_vertex_count + cap_vertex_count};
             vertex_index < mesh_data.normals.Num();
             ++vertex_index) {
            TestRunner->TestTrue(TEXT("Bottom cap normal points downward"),
                                 mesh_data.normals[vertex_index].Equals(-FVector3f::ZAxisVector));
        }
    }
    ...
    TEST_METHOD(GeneratesConsistentlyWoundTriangles)
    {
        ...
        for (int32 triangle_index{0}; triangle_index < triangle_count; ++triangle_index) {
            ...
            TestRunner->TestTrue(
                TEXT("Unreal front-face winding points away from the cylinder centre"),
    ...
    TEST_METHOD(GeneratesDeterministicCylinderData)
    {
        FSbxCylinderParameters const parameters{30.0f, 80.0f, 12};
        auto const first{generate_cylinder(parameters)};
        auto const second{generate_cylinder(parameters)};
