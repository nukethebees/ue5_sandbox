#include "ShooterGame/players/behaviour_tree/BTTask_PerformLookPattern.h"
...
auto UBTTask_PerformLookPattern::ExecuteTask(UBehaviorTreeComponent& owner_comp, uint8* node_memory)
    ...
    if (!pawn) {
        WARN_IS_FALSE(LogShooterGameAI, pawn);
...
void UBTTask_PerformLookPattern::TickTask(UBehaviorTreeComponent& owner_comp,
    ...
    if (reached_destination) {
        ...
        switch (move_state) {
            ...
            case ReturnToCentre: {
                UE_LOG(LogShooterGameAI, Verbose, TEXT("UBTTask_PerformLookPattern: Finished."));
...
auto UBTTask_PerformLookPattern::get_target_rotation(ELookPatternMoveState state) -> FQuat {
    switch (state) {
        ...
        default: {
            UE_LOG(LogShooterGameAI, Error, TEXT("%s"), *ml::make_unhandled_enum_case_warning(state));
            break;
        }
    }
