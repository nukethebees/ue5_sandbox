...
void UNpcPatrolComponent::advance_to_next_point() {
    ...
    if (!current_waypoint) {
        UE_LOG(LogShooterGameAI, Verbose, TEXT("No current waypoint. Using default."));
    ...
    if (!found) {
        UE_LOG(LogShooterGameAI, Warning, TEXT("Couldn't find the existing point."));
        set_waypoint_to_default();
        return;
    }
    ...
    if (next_index >= n) {
        UE_LOG(LogShooterGameAI, Verbose, TEXT("Looping to first waypoint."));
        current_waypoint = path[0];
    } else {
        UE_LOG(LogShooterGameAI, Verbose, TEXT("Moving to waypoint[%d]."), next_index);
        current_waypoint = path[next_index];
    }

    if (!current_waypoint) {
        UE_LOG(LogShooterGameAI, Warning, TEXT("Couldn't find the next waypoint."));
        set_waypoint_to_default();
    }
}
