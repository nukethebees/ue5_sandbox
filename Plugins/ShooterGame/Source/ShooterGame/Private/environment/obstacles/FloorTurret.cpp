#include "ShooterGame/environment/obstacles/FloorTurret.h"
...
void AFloorTurret::Tick(float dt) {
    ...
    switch (state.operating_state) {
        ...
        default: {
            UE_LOG(LogShooterGameActor, Warning, TEXT("Unhandled enum state."));
            break;
        }
    }
}
...
auto AFloorTurret::search_for_enemy(float vision_radius, float vision_angle) const -> AActor* {
    ...
    for (auto const& hit : hits) {
        ...
        UE_LOG(LogShooterGameAI, Verbose, TEXT("Found enemy: %s\n"), *target->GetActorLabel());
