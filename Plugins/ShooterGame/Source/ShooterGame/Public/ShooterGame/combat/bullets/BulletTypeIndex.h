...
struct FBulletTypeIndex {
  ...
  private:
    ...
    int32 value{};
  public:
    explicit FBulletTypeIndex(int32 v = 0)
        : value(v) {}
    template <typename... Args>
        requires std::constructible_from<int32, Args...>
    explicit FBulletTypeIndex(Args&&... args)
    ...
    friend uint32 GetTypeHash(FBulletTypeIndex const& obj) { return GetTypeHash(obj.value); }
};
