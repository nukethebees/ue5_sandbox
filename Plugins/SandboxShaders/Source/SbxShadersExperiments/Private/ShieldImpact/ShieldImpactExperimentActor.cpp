...
namespace {
constexpr TCHAR shield_impact_material_path[]{
...
void AShieldImpactExperimentActor::ensure_material() {
    if (!IsValid(shield_material_)) {
        shield_material_ = LoadObject<UMaterialInterface>(nullptr, shield_impact_material_path);
    }
    if (!IsValid(shield_material_)) {
        return;
    }
