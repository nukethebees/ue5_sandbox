...
The comparison manifest's `array-allocator` entries specify a type prefix and allocator type reference. The generator clones dynamic schemas and their nested references, then emits ordinary TArray-backed owners through the existing generator with an explicit allocator template argument. This is opt-in; production schemas are unchanged, and the native stdlib projection excludes the Unreal allocator variants.
