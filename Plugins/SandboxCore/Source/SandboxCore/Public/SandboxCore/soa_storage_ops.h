...
void reserve(Soa& value, int32 const count) {
    value.apply_arrays([count](auto&... columns) { ml::reserve(count, columns...); });
}

template <typename Soa>
void add_uninitialised(Soa& value, int32 const count) {
    value.apply_arrays([count](auto&... columns) { ml::add_uninitialised(count, columns...); });
}

template <typename Soa>
void add_defaulted(Soa& value, int32 const count) {
    value.apply_arrays([count](auto&... columns) { ml::add_defaulted(count, columns...); });
}
...
void remove_at_swap(Soa& value,
    ...
    value.apply_arrays(
        [=](auto&... columns) { ml::remove_at_swap(index, count, allow_shrinking, columns...); });
}

template <typename Soa>
void set_num(Soa& value, int32 const count, EAllowShrinking const allow_shrinking) {
    value.apply_arrays([=](auto&... columns) { ml::set_num(count, allow_shrinking, columns...); });
}
