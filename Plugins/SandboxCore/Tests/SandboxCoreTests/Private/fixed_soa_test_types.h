...
struct TTestFixedRowsArray {
    ...
    auto add(TArg0&& new_children_names, TArg1&& new_children_references, TArg2&& new_ids) -> size_type {
        return emplace_back(std::forward<TArg0>(new_children_names), std::forward<TArg1>(new_children_references), std::forward<TArg2>(new_ids));
    }

    void add_defaulted(size_type const count = 1)
        requires (std::is_default_constructible_v<FString> && std::is_default_constructible_v<TSharedPtr<int32>> && std::is_default_constructible_v<int32>)
    {
    ...
    auto capacity_view() -> View
        requires ((std::is_trivially_copyable_v<FString> && std::is_trivially_destructible_v<FString>) && (std::is_trivially_copyable_v<TSharedPtr<int32>> && std::is_trivially_destructible_v<TSharedPtr<int32>>) && (std::is_trivially_copyable_v<int32> && std::is_trivially_destructible_v<int32>))
    {
